...
/* forms list */
function html_showorigincode_contacts( $rows,$pageNav,$sort,$cat_row,$a,$form_styles){
	global $wpdb;
	?>
<div class="wrap">
	<?php origincode_contact_drawFreeBanner();?>
	<div id="poststuff">
		<div id="origincode_contacts-list-page">
			...
			<h2>OriginCode Forms
				<a onclick="window.location.href='<?php echo wp_nonce_url(admin_url('admin.php?page=origincode_forms_main_page&task=add_cat'), 'add_form', 'origincode_contact_add_form_nonce');?>'" class="add-new-h2" >Add New Form</a>
			</h2>
			<?php
			...
			$serch_fields='<div class="alignleft actions"">
									...
									<input type="text" name="search_events_by_title" value="'.$serch_value.'" id="search_events_by_title" onchange="origincode_contact_clear_search_texts()">
							</div>				
			 ...
			 origincode_contact_print_html_nav($pageNav['total'],$pageNav['limit'],$serch_fields);
			?>
			<table class="wp-list-table widefat fixed pages" style="width:100%">
				<thead>
				 <tr>
					<th scope="col" id="id" style="width:30px" ><span><?php _e('#','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th scope="col" id="name" style="width:85px" ><span><?php _e('Form Name','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th scope="col" id="prod_count"  style="width:75px;" ><span><?php _e('Fields Number','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th scope="col" id="shortcode"  style="width:75px;" ><span><?php _e('Shortcode','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th style="width:40px"><span><?php _e('Duplicate','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th style="width:40px"><span><?php _e('Delete','origincode_contact');?></span><span class="sorting-indicator"></span></th>
				 </tr>
				</thead>
				<tbody>
				 <?php 
				  ...
				  for($i=0; $i<count($rows);$i++){
					...
					if(isset($rows[$i+1]->id)){
					  ...
					  $edit_form_safe_link = wp_nonce_url(
						  'admin.php?page=origincode_forms_main_page&task=edit_cat&id=' . $rows[$i]->id,
						  'edit_form_' . $rows[$i]->id,
						  'origincode_contact_edit_form_nonce'
					  );
					$remove_form_safe_link = wp_nonce_url(
						admin_url('admin.php?page=origincode_forms_main_page&task=remove_cat&id=' . $rows[$i]->id.''),
						'remove_form_' . $rows[$i]->id,
						'origincode_forms_remove_form_nonce'
					);
					?>
					<tr <?php if ($trcount%2==0) { echo 'class="has-background"';}?>>
						...
						<td><?php echo '[origincode_forms id="'.$rows[$i]->id.'"]';?></td>
						<td>
							<a
								href="#"
								class="copy-field origincode_contact_duplicate_form"
								data-form-id="<?php echo $rows[$i]->id; ?>"
								data-nonce="<?php echo wp_create_nonce('duplicate_form_' . $rows[$i]->id) ?>"></a>
						</td>
						<td><a href="<?php echo $remove_form_safe_link; ?>" class="origincode_forms_delete_form"><span class="origincode-contact-delete"></span></a></td>
					</tr>
				  <?php } ?>
				</tbody>
			</table>
...
function origincode_contact_html_editorigincode_contact($current_form, $ord_elem, $count_ord,$images, $cat_row, $rowim, $rowsld, $paramssld, $rowsposts, $rowsposts8, $postsbycat, $form_styles,$style_values,$themeId){
 	global $wpdb;
 	$id=$current_form->id;

	if(isset($_GET["addslide"]) && $_GET["addslide"] == 1){
		$apply_safe_link = wp_nonce_url(
			'admin.php?page=origincode_forms_main_page&id=' . $id . '&task=apply',
			'apply_form_' . $id,
			'origincode_contact_apply_form_nonce'
		);
...
function submitbuttonSave(pressbutton){
	...
	if(!document.getElementById('origincode_contact_formname').value){
...
<div class="wrap">
	<?php
	origincode_contact_drawFreeBanner();
	$apply_safe_link = wp_nonce_url(
		'admin.php?page=origincode_forms_main_page&id=' . $id,
		'apply_form_' . $id,
		'origincode_contact_apply_form_nonce'
	);
	?>
		...
		<ul id="" class="origincode_contact_top_tabs">			
			...
			foreach ($rowsld as $rowsldires) :
				if ($rowsldires->id != $_GET['id']) :
					$edit_form_safe_link = wp_nonce_url(
						'admin.php?page=origincode_forms_main_page&task=edit_cat&id=' . $rowsldires->id,
				...
				<?php
					...
					<li class="active" onclick="this.lastElementChild.style.width = ((this.lastElementChild.value.length + 9) * 8) + 'px';" >
						...
						<input class="text_area" onfocus="this.style.width = ((this.value.length + 7) * 8) + 'px'" type="text" name="name" id="origincode_contact_formname" maxlength="250" value="<?php echo esc_html(stripslashes($current_form->name));?> " style="background:url(<?php echo plugins_url('../images/edit.png', __FILE__) ;?>) no-repeat #f3f4f8;" />
					</li>
				<?php	
				endif;
			endforeach;
			...
			<li class="add-new">
				<a onclick="window.location.href='<?php echo wp_nonce_url(admin_url('admin.php?page=origincode_forms_main_page&task=add_cat'), 'add_form', 'origincode_contact_add_form_nonce');?>'"></a>
			</li>
		</ul>
	</div>
	<div id="post-body" class="metabox-holder origin_contacts columns-2">
		<!-- Content -->
		...
		<?php add_thickbox(); ?>
			<div id="post-body-heading">
                <ul id="add-fields-block">
                    <li>
                        <button class="button" id="shortcode_toggle"><?php _e('Get Shortcode','origincode_contact');?></button>
                    </li>
                    <li class="spinnerLi" data-idForm="<?php echo $id;?>">
                        <img class="defSpin" src="<?php echo plugins_url( '../images/spinner.gif', __FILE__ ); ?>">
                    </li>
                    <li>
                        <strong><?php _e('Add Form Fields','origincode_contact');?></strong>
                        <ul id="add-default-fields">
                            <li><a onclick="" class="" id="text" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Text Box','origincode_contact');?></a><li>
                            <li><a onclick="" class="" id="textarea" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Textarea','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="e_mail" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Email','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="selectbox" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Selectbox','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="checkbox" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Checkbox','origincode_contact');?></a></li>
                            <li><a onclick=""  class="" id="radio_box" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Radio Box','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="file_box" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('File Box','origincode_contact');?></a></li>
                            <li><a onclick="submitbuttonSave('custom_text')" class="" id="custom_text" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Custom Text','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="hidden_field" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Hidden Field','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="page_break" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Page Break','origincode_contact');?></a></li>
							...
							<?php if ( $fordisablecaptcha == 0 ) :
								if ( $paramssld['form_captcha_public_key'] != '' and $paramssld['form_captcha_private_key'] != '' ) :
                                    ...
                                    <li class="bbdisabled">
                                        <a onclick="" class="captcha" id="captcha" data-formId="<?php echo $id; ?>" data-themeId="<?php echo $current_form->hc_yourstyle; ?>">
											<?php _e('Captcha','origincode_contact');?>
                                        </a>
                                    </li>
								<?php else : ?>
                                    <li class="bbdisabled">
                                        <a href="admin.php?page=origincode_forms_main_page&task=captcha_keys&id=<?php echo esc_html( $_GET['id'] ); ?>&TB_iframe=1"
                                ...
                                <li class="disabled"><a onclick="" class="captcha" id="captcha"
                                                        ...
                                                        data-themeId="<?php echo $current_form->hc_yourstyle; ?>"><?php _e('Captcha','origincode_contact');?></a>
                                </li>
							<?php endif;?>

                            <!-- simple captcha -->
							<?php if ( $disablesimplecaptcha == 0 ) : ?>
                                <li><a onclick="" class="" id="simple_captcha_box" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Simple Captcha','origincode_contact');?></a></li>
							<?php else : ?>
                                <li class="disabled"><a onclick="" class="" id="simple_captcha_box" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Simple Captcha','origincode_contact');?></a></li>
							<?php endif; ?>

							<?php if ( $fordisablebut == 0 ) : ?>
                                <li class=""><a onclick="" class="" id="buttons" data-formId="<?php echo $id; ?>" data-themeId="<?php echo $current_form->hc_yourstyle; ?>"><?php _e('Buttons','origincode_contact');?></a></li>
							<?php else : ?>
                                <li class="disabled"><a onclick="" class="" id="buttons" data-formId="<?php echo $id; ?>" data-themeId="<?php echo $current_form->hc_yourstyle; ?>"><?php _e('Buttons','origincode_contact');?></a></li>
							<?php endif; ?>
                        </ul>
                    </li>
                    <li class="spinnerLi" data-idForm="<?php echo $id;?>">
                    ...
                    <li>
                        ...
                        <ul id="add-default-fields" class="readyFields">
                            <li><a onclick="" class="" id="nameSurname" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Full Name','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="phone" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Phone','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="date" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Date','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="address" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Address','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="license" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Policy Agreement','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="license" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Paypal','origincode_contact');?></a></li>
                            <li><a onclick="" class="" id="license" data-formId="<?php echo $id;?>" data-themeId="<?php echo $current_form->hc_yourstyle;?>"><?php _e('Google Map','origincode_contact');?></a></li>
                        </ul>
                    </li>
                </ul>
                <div id="shortcode_fields">
                    <div class="short_incl"><label for="short_text"><?php _e('Shortcode','origincode_contact');?></label>
                        <textarea id="short_text" readonly="readonly">[origincode_forms id="<?php echo esc_html(stripslashes($id)); ?>"]</textarea>
                    </div>
                    <div class="templ_incl"><label for="temp_text"><?php _e('Template Include','origincode_contact');?></label>
                        <textarea id="temp_text" readonly="readonly">&lt;?php echo do_shortcode("[origincode_forms id='<?php echo esc_html(stripslashes($id)); ?>']"); ?&gt;</textarea>
                    </div>
                </div>
				<div id="save-button-block">
                    <div id="origc_options_block">
                        <div>
                            <label for="select_form_theme"><?php _e('Select Theme','origincode_contact');?></label>
                        ...
                        <div id="origc_sel_to_left">
                            <label for="select_form_show_title">Show Form Title</label>
                            <select id="select_form_show_title" name="origincode_contact_show_title_for_form_<?php echo $id; ?>">
                                <option value="default">Use Default Settings</option>
                                <option value="yes" <?php if (get_option('origincode_contact_show_title_for_form_' . $id) === 'yes') echo ' selected' ?>>Yes</option>
                                <option value="no" <?php if (get_option('origincode_contact_show_title_for_form_' . $id) === 'no') echo ' selected' ?>>No</option>
                            </select>
                        </div>
			...
			<?php
			function origincode_contact_origin_wptiny($initArray){
				...
				add_filter('tiny_mce_before_init', 'origincode_contact_origin_wptiny' );
				 ?>
            <div id="origc_n_btn_block">
					...
					if ( $rowimages->hc_left_right == 'left' ) {
						...
						switch ( $inputtype ) {
							case 'text':  //1
								...
								echo origincode_contact_textBoxSettingsHtml( $rowimages );
								break;

							case 'textarea':  //2

								echo origincode_contact_textareaSettingsHtml( $rowimages );
								break;
							case 'selectbox':  //3

								echo origincode_contact_selectboxSettingsHtml( $rowimages );
								break;
							case 'checkbox':  //4

								echo origincode_contact_checkboxSettingsHtml( $rowimages );
								break;
							case 'radio_box':  //5

								echo origincode_contact_radioboxSettingsHtml( $rowimages );
								break;
							case 'file_box':  //6

								echo origincode_contact_fileboxSettingsHtml( $rowimages );
								break;
							case 'custom_text':  //7

								echo origincode_contact_cutomtextSettingsHtml( $rowimages );
								break;
							case 'captcha':  //8

								echo origincode_contact_captchaSettingsHtml( $rowimages );
								break;
							case 'simple_captcha_box':  //8.1

								echo origincode_contact_simple_captcha_settings_html( $rowimages );
								break;
							case 'buttons':  //9

								echo origincode_contact_buttonsSettingsHtml( $rowimages );
								break;
							case 'e_mail':  //10

								echo origincode_contact_emailSettingsHtml( $rowimages );
								break;
                            case 'hidden_field':  //10

                                echo origincode_contact_hiddenFieldSettingsHtml( $rowimages );
                                break;
                            case 'page_break':  //10

                                echo origincode_contact_pageBreakSettingsHtml( $rowimages );
                                break;
						}
					}
				} ?>
			...
			#origincode-contact-wrapper > div {
			...
			#origincode-contact-wrapper label  {
			...
			#origincode-contact-wrapper .origincode-contact-column-block > div > label {
				display:block;
				width:38%;
				float:left;
				margin-right:2%;
			...
			#origincode-contact-wrapper label.error {
				color:#<?php echo $style_values['form_label_error_color']; ?>;
			}
			#origincode-contact-wrapper label em.required-star{
				color: #<?php echo $style_values['form_label_required_color']; ?>;
			}
			#origincode-contact-wrapper .origincode-contact-column-block > div .field-block ul li label span.sublable{vertical-align: super;}
			...
			/*FIELDS CUSTOM STYLES*/
				...
				#origincode-contact-wrapper.big-radio .radio-block i ,#origincode-contact-wrapper.big-checkbox .checkbox-block i {font-size:24px;}
				#origincode-contact-wrapper.medium-radio .radio-block i ,#origincode-contact-wrapper.medium-checkbox .checkbox-block i {font-size:20px;}
				#origincode-contact-wrapper.small-radio .radio-block i ,#origincode-contact-wrapper.small-checkbox .checkbox-block i {font-size:15px;}
				...
				/*############ License Field ############*/
				.origincode-check-field >.license-block{
					width: 100% !important;
				}
				.origincode-check-field > .license-block >.secondary-label{
					vertical-align: super !important;
				}
				.origincode-check-field > .license-block >.secondary-label > .checkbox-block{
			...
			<!--LIVE PREVIEW-->
			<div id="origincode-contact-preview-container">
					<form onkeypress="doNothing()" id="origincode-contact-preview-form">
					<div id="origincode-contact-wrapper" class="<?php echo $style_values['form_radio_size']; ?>-radio <?php echo $style_values['form_checkbox_size']; ?>-checkbox">
					<div <?php foreach ($rowim as $key=>$rowimages){
						...
						<?php
                            $show_form_title_option=get_option('origincode_contact_show_title_for_form_' . $id);
                        ...
                        echo '<h3 style="display: '.$display.';"><input class="text_area_title"  type="text" value="'.$current_form->name.'" /><span class="origincodeTitleOverlay"></span></h3>';
								...
								foreach ($rowim as $key=>$rowimages){
									...
									if($rowimages->hc_left_right == 'left'){
										...
										switch ($inputtype) {
											...
											echo origincode_contact_textBoxHtml($rowimages);
											break;

											case 'textarea'://2

											echo origincode_contact_textareaHtml($rowimages);
											break;

											case 'selectbox'://3
											
											echo origincode_contact_selectboxHtml($rowimages);
											break;

											case 'checkbox':  //4

											echo origincode_contact_checkboxHtml($rowimages,$themeId);
											break;

                                            case 'hidden_field':  //4

                                            echo origincode_contact_hiddenFieldHtml($rowimages,$themeId);
                                            break;

                                            case 'page_break':  //4

                                            echo origincode_contact_pageBreakHtml($rowimages,$themeId);
                                            break;

											case 'radio_box':  //5
											
											echo origincode_contact_radioboxHtml($rowimages,$themeId);
											break;
											case 'file_box':  //6
											
											echo origincode_contact_fileboxHtml($rowimages,$themeId);
											break;

											case 'custom_text':  //7

											echo origincode_contact_cutomtextHtml($rowimages);
											break;

											case 'captcha': //8
											?>
												<div class="origincode-field-block captcha-block" rel="origin-contact-field-<?php echo $rowimages->id; ?>">
													<script type="text/javascript">
											...
											case 'simple_captcha_box': //8.1
												?>
												<?php if($rowimages->hc_input_show_default=='formsLeftAlign'){$origc_left_right_class='text-left';}
											    else{$origc_left_right_class='text-right';}?>
                                                <?php $hc_other_field = json_decode($rowimages->hc_other_field);?>
												<div class="origincode-field-block simple-captcha-block <?php echo $origc_left_right_class;?>" rel="origin-contact-field-<?php echo $rowimages->id; ?>">
													<?php $capPos='right';if($rowimages->hc_input_show_default=='2')$capPos="left";?>
													<label  class="formsAboveAlign">
                                                        <img src="<?php echo origincode_contact_create_new_captcha($rowimages->id,'admin');?>">
                                                        <span class="origincode_captcha_refresh_button" data-captcha-id="<?php echo $rowimages->id;?>" data-digits="<?php echo (isset($hc_other_field->digits))?$hc_other_field->digits:5;?>" data-form-id="<?php echo $current_form->id; ?>">
											...
											echo origincode_contact_buttonsHtml($rowimages,$themeId);
											break;

											case 'e_mail':  //10

											echo origincode_contact_emailHtml($rowimages);
											break;
										}
									}
								}
						...
						<div class="origincode-contact-column-block origincode-contact-block-right" id="origincode-contact-block-right">
						...
						<?php	
								...
								foreach ($rowim as $key=>$rowimages){
									if($rowimages->hc_left_right == 'right'){
										...
										switch ($inputtype) {
											...
											//here
											case 'simple_captcha_box'://8.1
												echo origincode_contact_simple_captcha_html($rowimages,$themeId);
												break;
								...
								$origincode_contact_form_admin_subject = get_option('origincode_contact_form_admin_subject_' . $id);
								$origincode_contact_form_admin_subject = $origincode_contact_form_admin_subject ? $origincode_contact_form_admin_subject : '';

								$origincode_contact_form_user_subject = get_option('origincode_contact_form_user_subject_' . $id);
								$origincode_contact_form_user_subject = $origincode_contact_form_user_subject ? $origincode_contact_form_user_subject : '';
							?>
						</div>
					<div class="clear"></div>
					</div>
			...
			<div class="origincode_contact_custom_settings_main">
				<div class="origincode_contact_custom_settings_dropdown_heading_wrapper">
                    <div class="origincode_contact_custom_settings_dropdown_heading"><h4>Advanced email options (Pro)</h4><i class="originicons-chevron-down"> </i></div>
					<div class="origincode_contact_custom_settings_dropdown_description">In your LITE version of the plugin you can send ONE custom email message and set the same admin recipients for ALL forms, whereas Advanced email options in PRO version allow you to customize your email messages and admin recipients for EACH form.</div>
				</div>
				<div class="origincode_contact_custom_settings_dropdown_content -hidden">
					<div class="origincode_contact_custom_settings_outer_wrapper">
						<img src="<?php echo plugins_url('../images/origincode_contact_pro_advanced_options.png',__FILE__);?>" width="100%">
					</div>
				</div>
			</div>
			<!-- ################################################ LIVE PREVIEW GOESE TO FRONT END #################################################### -->
...
?>
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {			
			...
			jQuery('.origincode-insert-post-button').on('click', function() {
				var ID1 = jQuery('#origincode_add_video_input').val();
	...
	<div id="origincode_contacts_captcha_keys">
		<div id="origincode_contacts_captcha_keys_wrap">
			<h2>Captcha Keys</h2>
			<div class="control-panel">
				...
				<form method="post" action="admin.php?page=origincode_forms_main_page" id="adminFormPopup" name="admin_form">
					<div>
				...
				<p>Please be known you may always change it from <a target="blank" href="admin.php?page=origincode_forms_general_options&closepop=1">General Options</a></p>
			</div>
		</div>	
	</div>
	<script>
