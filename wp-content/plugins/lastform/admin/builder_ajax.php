...
if ( ! defined( 'ABSPATH' ) ) exit;
	global $wpdb;
	//Draw the form
	function origincode_contact_drawThemeNew($themeId) { ob_start();
		global $wpdb;
		$query = "SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
	...
	function origincode_contact_textBoxHtml($rowimages) { ob_start(); ?>
	    <div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			...
			<div class="field-block input-text-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign'||$rowimages->hc_input_show_default=='formsInsideAlign')echo $rowimages->hc_input_show_default;?>">
				...
				<input id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" type="text" placeholder="<?php echo $placeholder;?>" class="<?php if($rowimages->hc_required == 'on'){echo 'required';}?>"  <?php if($rowimages->description != 'on'){echo 'disabled="disabled"';}?>/>
			</div>
			<span class="origincode-error-message"></span>
			<span class="originOverlay"></span>
...
function origincode_contact_textBoxSettingsHtml($rowimages){ ob_start(); ?>
    <li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
        ...
        <div class="fields-options">
            ...
            <!--Mask On-->
            <label class="input-block"><?php _e('Mask On','origincode_contact'); ?>
                  <input type="checkbox" <?php if($rowimages->field_type == 'number') echo "disabled";?> class="origc-mask-on-check" <?php if(!empty(trim($rowimages->mask_on)) && $rowimages->field_type == 'text') echo "checked='checked'";  ?>>
                  <label class="origc-mask-on <?php if(empty(trim($rowimages->mask_on)) || $rowimages->field_type == 'number') echo "readonlyHgMask" ?>" >
            ...
            <div class="left">
                ...
                <div>
               <!-- Default Value -->
              <label class="input-block"><?php _e('Default Value','origincode_contact'); ?>
                  <input class="origc-def-value"  type="text" name="def_value<?php echo absint($rowimages->id); ?>"  value="<?php echo $rowimages->def_value; ?>" <?php if(!empty(trim($rowimages->mask_on))) echo "readonly='readonly'"; ?>  />
              </label>
                <!-- Default Value -->
                </div>
	...
	//2 Textarea //
    function origincode_contact_textareaHtml($rowimages) { ob_start(); ?>
    	<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?></label>
			<div class="field-block textarea-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign'||$rowimages->hc_input_show_default=='formsInsideAlign')echo esc_html($rowimages->hc_input_show_default);?>">
				...
				<textarea style="height:<?php echo $rowimages->hc_other_field;?>px;resize:<?php if($rowimages->field_type=='on'){echo 'vertical';}else{ echo 'none';}?>;" id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" <?php if($rowimages->description != 'on'){echo 'disabled="disabled"';}?> placeholder="<?php echo esc_html($placeholder); ?>"></textarea>
			</div>
			<span class="origincode-error-message"></span>
			<span class="originOverlay"></span>
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>">
	...
	function origincode_contact_textareaSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//3 Selectbox //
	function origincode_contact_selectboxHtml($rowimages) { ob_start(); ?>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?></label>
			<div class="field-block selectbox-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign' || $rowimages->hc_input_show_default=='formsInsideAlign')echo esc_html($rowimages->hc_input_show_default);?>" >
				...
				<select id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" >
					<?php
	...
	function origincode_contact_selectboxSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//4 Checkbox //
	function origincode_contact_checkboxHtml($rowimages,$themeId) { ob_start();
		global $wpdb;
	...
	function origincode_contact_checkboxSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
...
function origincode_contact_hiddenFieldHtml($rowimages,$themeId) { ob_start();
    global $wpdb;
    $themeId = sanitize_text_field($themeId);
    $query = $wpdb->prepare("SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = %s", $themeId);
    $rows = $wpdb->get_results($query);
    $style_values = array();
    foreach ($rows as $row) {
        $key = $row->name;
    ...
    <div class="origincode-field-block origincode-check-field" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>" style="background-color: rgba(211, 211, 211, 0.45) !important; font-weight: bold !important; border-radius: 3px !important;">
        <label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>">
...
function origincode_contact_hiddenFieldSettingsHtml($rowimages) { ob_start(); ?>
    <li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
        <input type="hidden" class="left-right-position"  name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" />
        <input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
        ...
        <div class="fields-options">
            ...
            <div class="left">
                <div>
                    <?php _e('Default Value: ','origincode_contact');?><br>
...
function origincode_contact_pageBreakHtml($rowimages,$themeId) { ob_start();
    global $wpdb;
    $themeId = sanitize_text_field($themeId);
    $query = "SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
    $rows = $wpdb->get_results($query);
    ...
    <div class="origincode-field-block origincode-check-field" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>"  style="text-align: center!important;font-weight: bold!important;">
        <hr>
        <label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" style="width:100%!important;">
...
function origincode_contact_pageBreakSettingsHtml($rowimages) { ob_start(); ?>
    <li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
        <input type="hidden" class="left-right-position"  name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" />
        <input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
	...
	//5 Radiobox //
	function origincode_contact_radioboxHtml($rowimages, $themeId) { ob_start();
		$themeId = sanitize_text_field($themeId);
		global $wpdb;
		$query = "SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
	    $rows = $wpdb->get_results($query);
		...
		<div class="origincode-field-block origincode-radio-field" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';}?></label>
			<div class="field-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign')echo esc_html($rowimages->hc_input_show_default);?>">
				<ul id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>">
					<?php
					 $options=explode(';;',$rowimages->name);
					 $i=0;
	...
	function origincode_contact_radioboxSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"   data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//6 Filebox //
	function origincode_contact_fileboxHtml($rowimages,$themeId) { ob_start();
		$themeId = sanitize_text_field($themeId);
		global $wpdb;
		$query = "SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
	    $rows = $wpdb->get_results($query);
		...
		jQuery(document).ready(function(){
			...
			jQuery(".origincode-contact-column-block input[name='MAX_FILE_SIZE']").attr('value',byteRes);
		});													
		</script>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo $rowimages->hc_input_show_default;?>" for="origincode_preview_textbox_<?php echo htmlspecialchars($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?></label>
			<div class="field-block file-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign'||$rowimages->hc_input_show_default=='formsInsideAlign')echo esc_html($rowimages->hc_input_show_default);?>">
				...
				<input id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" type="file" name="userfile"/>
			</div>
			<span class="origincode-error-message"></span>
			<span class="originOverlay"></span>
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>">
	...
	function origincode_contact_fileboxSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//7 Custom Text //
	function origincode_contact_cutomtextHtml($rowimages) { ob_start(); ?>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
	...
	function origincode_contact_cutomtextSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"   data-fieldNum="<?php echo absint($rowimages->id); ?>" data-fieldType="custom_text">
			<input type="hidden" class="left-right-position"   name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" />
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
			<h4>Custom Text</h4>
			<div class="fields-options">	
		...
		<script>
					function editorchange(value){
						...
						var previewfield=jQuery('.origincode-contact-column-block > div[rel="'+fieldid+'"]');										
	...
	//8 ReCaptcha //
	function origincode_contact_captchaHtml($rowimages) { ob_start(); ?>
	...
	function origincode_contact_captchaSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>" data-fieldType="captcha">
			<input type="hidden" class="left-right-position" name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" />
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
			<h4>ReCaptcha</h4>
			<div class="fields-options">
				...
				<div class="left">
					<label class="input-block">ReCaptcha Theme
						<select class="origincode_contact_captcha_theme" name="hc_required<?php echo absint($rowimages->id); ?>">
...
function origincode_contact_simple_captcha_html($rowimages,$formid) { ob_start(); ?>
	...
	<div class="origincode-field-block simple-captcha-block <?php echo esc_html($capPos);?>" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
		<label class="formsAboveAlign">
			<img src="<?php echo origincode_contact_create_new_captcha($rowimages->id,'admin');?>">
			<span class="origincode_captcha_refresh_button" data-captcha-id="<?php echo absint($rowimages->id);?>" data-digits="<?php echo isset($hc_other_field->digits)?esc_html($hc_other_field->digits):5;?>" data-form-id="<?php echo esc_html($formid); ?>">
...
function origincode_contact_simple_captcha_settings_html($rowimages) { ob_start(); ?>
	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"  data-fieldNum="<?php echo absint($rowimages->id); ?>" data-fieldType="simple_captcha_box">
	...
	//9 Buttons //
	function origincode_contact_buttonsHtml($rowimages,$themeId) { ob_start();
		$themeId = sanitize_text_field($themeId);
		global $wpdb;
		$query = "SELECT * from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
	    $rows = $wpdb->get_results($query);
		...
		?>
		<div class="origincode-field-block buttons-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<button type="submit" class="submit" id="origincode_preview_button__submit_<?php echo absint($rowimages->id);?>" value="Submit">
			...
			<button type="reset" class="reset" <?php if($rowimages->hc_required!='checked') echo 'style="display: none;"'?> id="origincode_preview_button_reset_<?php echo absint($rowimages->id);?>" value="Reset">
	...
	function origincode_contact_buttonsSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>"   data-fieldNum="<?php echo absint($rowimages->id); ?>" data-fieldType="buttons">
			<input type="hidden" class="left-right-position" name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" />
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
			<h4>Buttons</h4>
			<div class="fields-options">
	...
	//10 Email //
	function origincode_contact_emailHtml($rowimages) { ob_start(); ?>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?> </label>
			<div class="field-block input-text-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign'||$rowimages->hc_input_show_default=='formsInsideAlign')echo esc_html($rowimages->hc_input_show_default);?>">
				...
				<input id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" type="email" placeholder="<?php echo esc_html($placeholder);?>" class="<?php if($rowimages->hc_required == 'on'){echo 'required';}?>"  <?php if($rowimages->description != 'on'){echo 'disabled="disabled"';}?>/>
			</div>
			<span class="origincode-error-message"></span>
			<span class="originOverlay"></span>
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>">
	...
	function origincode_contact_emailSettingsHtml($rowimages) { ob_start(); ?>
    	<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>" data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//11 Name
	function origincode_contact_nameSurnameHtml($rowimages) { ob_start(); ?>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?> </label>
			<div class="field-block input-name-block <?php if($rowimages->hc_input_show_default=='formsAboveAlign'||$rowimages->hc_input_show_default=='formsLabelHide')echo esc_html($rowimages->hc_input_show_default);?>">
				<input id="origincode_preview_textbox_<?php echo absint($rowimages->id);?>" type="text" placeholder="<?php echo esc_html($rowimages->name); ?>" class="pl_name <?php if($rowimages->hc_required == 'on'){echo 'required';}?>"  <?php if($rowimages->description != 'on'){echo 'disabled="disabled"';}?>/>
				<input id="origincode_preview_textbox_1<?php echo absint($rowimages->id);?>" type="text" placeholder="<?php echo esc_html($rowimages->hc_other_field); ?>" class="pl_surname <?php if($rowimages->hc_required == 'on'){echo 'required';}?>"  <?php if($rowimages->description != 'on'){echo 'disabled="disabled"';}?>/>
			</div>
			<span class="origincode-error-message"></span>
			<span class="originOverlay"></span>
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>">
	...
	function origincode_contact_nameSurnameSettingsHtml($rowimages){ob_start(); ?>
		<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>" data-fieldNum="<?php echo absint($rowimages->id); ?>">
	...
	//12 Phone
	function origincode_contact_phoneHtml($rowimages) { ob_start(); ?>
		<div class="origincode-field-block" rel="origin-contact-field-<?php echo absint($rowimages->id); ?>">
			<label class="<?php if($rowimages->hc_input_show_default!='1')echo esc_html($rowimages->hc_input_show_default);?>" for="origincode_preview_textbox_<?php echo absint($rowimages->id);?>"><?php echo esc_html($rowimages->hc_field_label); if($rowimages->hc_required == 'on'){ echo '<em class="required-star">*</em>';} ?> </label>
...
esc_html($rowimages->hc_input_show_default);?>">
	...
	function origincode_contact_phoneSettingsHtml($rowimages){ob_start(); ?>
		<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>" data-fieldNum="<?php echo absint($rowimages->id); ?>">
			<input type="hidden" class="left-right-position" name="hc_left_right<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->hc_left_right); ?>" fileType="nameSurname"/>
			<input type="hidden" class="ordering" name="hc_ordering<?php echo absint($rowimages->id); ?>" value="<?php echo esc_html($rowimages->ordering); ?>" />
	...
	//13 License
	function origincode_contact_licenseHtml($rowimages,$themeId) { ob_start();
		$themeId = sanitize_text_field($themeId);
		global $wpdb;
		$query = "SELECT *  from " . $wpdb->prefix . "origincode_contact_style_fields where options_name = '".$themeId."' ";
	    $rows = $wpdb->get_results($query);
	...
	function origincode_contact_licenseSettingsHtml($rowimages){ob_start(); ?>
		<li id="origin-contact-field-<?php echo absint($rowimages->id); ?>" data-fieldNum="<?php echo absint($rowimages->id); ?>"  data-fieldType="license">	
	...
	if (isset($_POST['task']) && $_POST['task']=='addFieldsTask') {
		...
		$inserttexttype = $wpdb->prefix . "origincode_contact_contacts_fields";
		switch ($inputtype) {
		    case 'text':
		        $inserttexttype = $wpdb->prefix . "origincode_contact_contacts_fields";
		        ...
		        $queryMax=$wpdb->prepare(
					...
					FROM ".$wpdb->prefix."origincode_contact_contacts_fields 
					WHERE origincode_contact_id=%d",
			        $formId
		        );
			    $row8=$wpdb->get_results($queryMax);
			    $fieldID= absint($row8[0]->resId);
			    $fieldQuery=$wpdb->prepare("SELECT * FROM ".$wpdb->prefix."origincode_contact_contacts_fields WHERE id=%d",$fieldID);
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_textBoxHtml($rowimages[0]),
				    "outputFieldSettings" => origincode_contact_textBoxSettingsHtml($rowimages[0])
			    ));
		    ...
		    case 'textarea':
			    ...
			    $queryMax=$wpdb->prepare(
			    	"SELECT MAX(id) AS resId 
					FROM ".$wpdb->prefix."origincode_contact_contacts_fields 
					WHERE origincode_contact_id=%d",$formId
			    );
			    $row8=$wpdb->get_results($queryMax);
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_textareaHtml($rowimages[0]),
                    "outputFieldSettings" =>origincode_contact_textareaSettingsHtml($rowimages[0])
			    ));
		        break;
		    ...
		    case 'selectbox':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_selectboxHtml($rowimages[0]),
				    "outputFieldSettings" =>origincode_contact_selectboxSettingsHtml($rowimages[0])
			    ));
		     	break;
			...
			case 'checkbox':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_checkboxHtml($rowimages[0],$themeId),
				    "outputFieldSettings" =>origincode_contact_checkboxSettingsHtml($rowimages[0])
			    ));
		     	break;
            ...
            case 'hidden_field':
                ...
                $rowimages=$wpdb->get_results($fieldQuery);
                echo json_encode(array(
                    "outputField" => origincode_contact_hiddenFieldHtml($rowimages[0],$themeId),
                    "outputFieldSettings" =>origincode_contact_hiddenFieldSettingsHtml($rowimages[0])
                ));
                break;
            ...
            case 'page_break':
                ...
                $rowimages=$wpdb->get_results($fieldQuery);
                echo json_encode(array(
                    "outputField" => origincode_contact_pageBreakHtml($rowimages[0],$themeId),
                    "outputFieldSettings" =>origincode_contact_pageBreakSettingsHtml($rowimages[0])
                ));
                break;
	     	...
	     	case 'radio_box':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_radioboxHtml($rowimages[0],$themeId),
				    "outputFieldSettings" =>origincode_contact_radioboxSettingsHtml($rowimages[0])
			    ));
		     	break;
	     	...
	     	case 'file_box':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_fileboxHtml($rowimages[0],$themeId),
				    "outputFieldSettings" =>origincode_contact_fileboxSettingsHtml($rowimages[0])
			    ));
		     	break;
	     	...
	     	case 'custom_text':
			    ...
			    $row8=$wpdb->get_results($queryMax);
			    $fieldID=$row8[0]->resId;
			    echo json_encode(array( "outputField" => origincode_contact_cutomtextHtml($rowimages[0]),
                    "outputFieldSettings" =>origincode_contact_cutomtextSettingsHtml($rowimages[0]),
	     	...
	     	case 'captcha':
			    ...
			    if($capKeyPub != '' && $capKeyPriv != '') {
				    ...
				    $capKeyPub=$paramssld['form_captcha_public_key'];
				    echo json_encode(array(
				    	"outputField" => origincode_contact_captchaHtml($rowimages[0]),
					    "outputFieldSettings" => origincode_contact_captchaSettingsHtml($rowimages[0]),
					    "captchaNum" => $capKeyPub
				    ));
				} else {
					echo json_encode(array(
						"captchaNum" => $capKeyPub,
						"toRedirect"=>"admin.php?page=origincode_forms_main_page&task=captcha_keys&id=".$formId."&TB_iframe=1"
					));
				}
		     	break;
			case 'simple_captcha_box'://simple captcha to do
				$field_exists_in_the_form=$wpdb->prepare(
					"SELECT id 
					FROM ".$wpdb->prefix."origincode_contact_contacts_fields 
					WHERE conttype='simple_captcha_box' AND origincode_contact_id=%d",
					$formId
				);
				...
				if( !$field_exists_in_the_form ){
					...
					$rowimages=$wpdb->get_results($fieldQuery);
					echo json_encode(array(
						"outputField" => origincode_contact_simple_captcha_html($rowimages[0]),
						"outputFieldSettings"=>origincode_contact_simple_captcha_settings_html($rowimages[0])
					));
				}
				else{
					echo json_encode(array());
	     	...
	     	case 'buttons':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_buttonsHtml($rowimages[0],$themeId),
                    "outputFieldSettings" => origincode_contact_buttonsSettingsHtml($rowimages[0]),
				    "buttons" => 'button'
			    ));
		     	break;
	     	...
	     	case 'e_mail':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_emailHtml($rowimages[0]),
                    "outputFieldSettings" => origincode_contact_emailSettingsHtml($rowimages[0])
			    ));
		     	break;
	     	...
	     	case 'nameSurname':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_nameSurnameHtml($rowimages[0]),
				    "outputFieldSettings"=>origincode_contact_nameSurnameSettingsHtml($rowimages[0])
			    ));
		     	break;
	     	...
	     	case 'phone':
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_phoneHtml($rowimages[0]),
				    "outputFieldSettings"=>origincode_contact_phoneSettingsHtml($rowimages[0])
			    ));
		     	break;
	     	...
	     	case 'license':
		        ...
		        $wpdb->insert(
			        ...
			        array(
				        'name' => 'License',
				        'origincode_contact_id' => $formId,
				        'description' => 'http://origincode.co',
				        'conttype' => $inputtype,
			    ...
			    $rowimages=$wpdb->get_results($fieldQuery);
			    echo json_encode(array(
			    	"outputField" => origincode_contact_licenseHtml($rowimages[0],$themeId),
				    "outputFieldSettings"=>origincode_contact_licenseSettingsHtml($rowimages[0])
			    ));
		     	break;
	...
	if( isset($_POST['task']) && $_POST['task']=='removeFieldTask') {
		...
		$formId = absint($_POST['formId']);
		$all = sanitize_text_field($_POST['formData']);
		parse_str("$all",$myArray);
		$fieldID = absint($_POST['fieldId']);
		$_POSTED= array_map('sanitize_text_field', $myArray);
		$query=$wpdb->prepare("SELECT * FROM ".$wpdb->prefix."origincode_contact_contacts_fields where origincode_contact_id = %d order by id ASC",$formId);
	    $rowim=$wpdb->get_results($query);
		if(isset($_POSTED["name"])){
			if($_POSTED["name"] != ''){
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_contacts SET  name = %s  WHERE id = %d ", sanitize_text_field($_POSTED["name"]), $formId));
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_contacts SET  hc_yourstyle = %s WHERE id = %d ", sanitize_text_field($_POSTED["select_form_theme"]), $formId));
			}
		}	   
			   ...
			   if($_POSTED["hc_left_right".$rowimages->id.""]){
			   	$id = absint($rowimages->id);
					if ( isset( $_POSTED[ "field_type" . $rowimages->id . "" ] ) )
						$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  field_type = %s WHERE id = %d", sanitize_text_field($_POSTED[ "field_type" . $rowimages->id . "" ]), $id ) );
				   if ( isset( $_POSTED[ "hc_other_field" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  hc_other_field = %s WHERE id = %d", sanitize_text_field($_POSTED[ "hc_other_field" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "titleimage" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  name = %s  WHERE id = %d", sanitize_text_field(stripslashes( $_POSTED[ "titleimage" . $rowimages->id . "" ] )), $id ) );
				   }
				   if ( isset( $_POSTED[ "im_description" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  description = %s  WHERE id = %d", sanitize_text_field($_POSTED[ "im_description" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "hc_required" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  hc_required = %s WHERE id = %d", sanitize_text_field($_POSTED[ "hc_required" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "imagess" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  hc_field_label = %s  WHERE id = %d", sanitize_text_field(stripslashes( $_POSTED[ "imagess" . $rowimages->id . "" ] )), $id ) );
				   }
				   if ( isset( $_POSTED[ "hc_left_right" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  hc_left_right = %s  WHERE id = %d", sanitize_text_field($_POSTED[ "hc_left_right" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "def_value" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  def_value = %s  WHERE id = %d", sanitize_text_field($_POSTED[ "def_value" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "mask_on" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  mask_on = %s  WHERE id = %d", sanitize_text_field($_POSTED[ "mask_on" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED[ "hc_ordering" . $rowimages->id . "" ] ) ) {
					   $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  ordering = %s  WHERE id = %d", intval($_POSTED[ "hc_ordering" . $rowimages->id . "" ]), $id ) );
				   }
				   if ( isset( $_POSTED["hc_input_show_default".$rowimages->id.""]))$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_contacts_fields SET  hc_input_show_default = %s  WHERE id = %d",sanitize_text_field($_POSTED["hc_input_show_default".$rowimages->id.""]),$id));
				}
			}
		}
		$wpdb->delete(
	...
	if (isset($_POST['task']) && $_POST['task'] == 'dublicateFieldTask'){
			...
			if ( isset( $_POSTED ) && isset( $_POSTED[ "hc_left_right" . $rowimages->id . "" ] ) ) {
				if ( $_POSTED[ "hc_left_right" . $rowimages->id . "" ] ) {
					...
					if ( isset( $_POSTED[ "hc_input_show_default" . $rowimages->id . "" ] ) ) {
						$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET  hc_input_show_default = %s  WHERE id = %d", sanitize_text_field($_POSTED[ "hc_input_show_default" . $rowimages->id . "" ]), $id ) );
					}
				}
			}
		}
		///ENDS SAVING
		$query=$wpdb->prepare("SELECT * FROM ".$wpdb->prefix."origincode_contact_contacts_fields WHERE id=%d",$fieldID);
		...
		foreach ( $rowplusorder as $key => $rowplusorders ) {
			if ( $rowplusorders->ordering > $rowduble->ordering ) {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields SET ordering = %d WHERE id = %d ", $rowplusorderspl, $rowplusorders->id ) );
			}
		}
		...
		$wpdb->insert(
			...
			array(
				'name' => $rowduble->name,
				'origincode_contact_id' => $rowduble->origincode_contact_id,
		...
		$queryMax=$wpdb->prepare("SELECT MAX(id) AS resId FROM ".$wpdb->prefix."origincode_contact_contacts_fields WHERE origincode_contact_id=%d",$formId);
	    $row8=$wpdb->get_results($queryMax);
	    $fieldID2=$row8[0]->resId;
	    $fieldQuery=$wpdb->prepare("SELECT * FROM ".$wpdb->prefix."origincode_contact_contacts_fields WHERE id=%d",$fieldID2);
	    $rowimages=$wpdb->get_results($fieldQuery);
		...
		switch ( $inputtype ) {
			...
			case 'custom_text':
				echo json_encode( array(
					"outputField"         => origincode_contact_cutomtextHtml( $rowimages[0] ),
					"outputFieldSettings" => origincode_contact_cutomtextSettingsHtml( $rowimages[0] ),
					"customText"          => "titleimage" . $fieldID . "",
					"beforeId"            => $fieldID,
				) );
...
if ( isset( $_POST['task'] ) && $_POST['task'] == 'saveEntireForm' ) {
    ...
    if ( isset( $_POSTED ) ) {
        if (isset($_POSTED["name"])) {
            if ($_POSTED["name"] != '') {
                $wpdb->query($wpdb->prepare("UPDATE " . $wpdb->prefix . "origincode_contact_contacts SET  name = %s  WHERE id = %d ", sanitize_text_field(wp_unslash($_POSTED["name"])), $formId));
                $wpdb->query($wpdb->prepare("UPDATE " . $wpdb->prefix . "origincode_contact_contacts SET  hc_yourstyle = %s  WHERE id = %d ", sanitize_text_field($_POSTED["select_form_theme"]), $formId));
                $wpdb->query($wpdb->prepare("UPDATE " . $wpdb->prefix . "origincode_contact_contacts SET  hc_yourstyle = %s  WHERE id = %d ", sanitize_text_field($_POSTED["select_form_theme"]), $formId));
            }
        }

        if (isset($_POSTED['origincode_contact_show_title_for_form_' . $formId]) && in_array($_POSTED['origincode_contact_show_title_for_form_' . $formId], array('yes', 'no', 'default'))) {
            update_option('origincode_contact_show_title_for_form_' . $formId, $_POSTED['origincode_contact_show_title_for_form_' . $formId]);
        }
...
/* End Save Entire Form */
	...
	if (isset($_POST['task'])&&$_POST['task']=='changeFormTheme') {
		...
		if ( isset( $_POSTED["name"] ) ) {
			if ( $_POSTED["name"] != '' ) {
				$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts SET  name = %s  WHERE id = %d ", sanitize_text_field(wp_unslash($_POSTED["name"])), $formId ) );
				$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_contacts SET  hc_yourstyle = %s  WHERE id = %d ", intval($themeId), $formId ) );
			}
		}
		foreach ( $rowim as $key => $rowimages ) {
			if ( isset( $_POSTED ) && isset( $_POSTED[ "hc_left_right" . $rowimages->id . "" ] ) ) {
				if ( $_POSTED[ "hc_left_right" . $rowimages->id . "" ] ) {
					...
					if ( isset( $_POSTED[ "field_type" . $rowimages->id . "" ] ) ) {
						$wpdb->query( $wpdb->prepare(
							"UPDATE " . $wpdb->prefix . "origincode_contact_contacts_fields 
							SET  field_type = %s 
		...
		echo origincode_contact_drawThemeNew($themeId);
	}
/* Change Theme */
