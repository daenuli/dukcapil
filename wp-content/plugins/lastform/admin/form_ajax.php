...
function origincode_contact_ajax_action_callback(){
	global $wpdb;
////////////////////////SUBMISSION PAGE////////////////////////BEGIN
	...
	if( isset($_POST['task']) && $_POST['task']=='moveTospamSubmitions' ){
		...
		$arrayOfids=sanitize_text_field($_POST['spam_submitions']);
		$allNumbers = true;
		...
		if($allNumbers){
			foreach ($arrayOfids as $arrayOfid) {
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_spam = '%d'  WHERE id = '%d' ", 1, $arrayOfid));
			}
		}		
		return;
	}
	...
	if(isset($_POST['task'])&&$_POST['task']=='moveToSpamSingleSubmition'){
		...
		if(is_numeric($subId)){
			$subId = absint($subId);
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_spam = '%d'  WHERE id = '%d' ", 1, $subId));
		}			
		return;
	}
	// Unmark as Spam
	if(isset($_POST['task'])&&$_POST['task']=='moveFromspamSubmitions'){
		...
		if($allNumbers){
			foreach ($arrayOfids as $arrayOfid) {
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_spam = '%d'  WHERE id = '%d' ", 0, $arrayOfid));
			}
		}	
		return;
	}
	...
	if( isset($_POST['task']) && $_POST['task']=='moveFromSpamSingleSubmition' ){
		...
		if( is_numeric($subId) ){
			$subId = absint($subId);
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_spam = '%d'  WHERE id = '%d' ", 0, $subId));
		}
		return;
	}
	// Delete
	if( isset($_POST['task']) && $_POST['task']=='deleteSubmitions' ){
		...
		$arrayOfids=sanitize_text_field($_POST['submitions_for_delete']);
		$allNumbers = true;
		foreach ( $arrayOfids as &$item ) {
			$item = absint($item);
		    if ( ! is_numeric($item) ) {
		...
		if( $allNumbers ) {
			foreach ($arrayOfids as $arrayOfid) {
				...
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."origincode_contact_submission WHERE id=%d",$arrayOfid));
			}
		}		
		return;
	}
	...
	if( isset($_POST['task']) && $_POST['task']=='deleteSingleSubmition' ){
		...
		$subId=absint($_POST['submissionId']);
		if( is_numeric($subId) ){
			$subId = absint($subId);
			$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."origincode_contact_submission WHERE id=%d",$subId));
		}	
		return;
	}
	// Mark as Read
	if( isset($_POST['task'])&&$_POST['task']=='markAsRead' ){
		...
		if($allNumbers==true){
			foreach ($arrayOfids as $arrayOfid) {
				$arrayOfid = absint($arrayOfid);
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_read_or_not = '%d'  WHERE id = '%d' ", 1, $arrayOfid));
			}
		}
		return;
	}
	...
	if(isset($_POST['task'])&&$_POST['task']=='markAsUnread'){
		...
		$arrayOfids=sanitize_text_field($_POST['unread_submitions']);
		$allNumbers = true;
		foreach ($arrayOfids as $item) {
		    if (!is_numeric($item)) {
		        $allNumbers = false;
		...
		if($allNumbers==true){
			foreach ($arrayOfids as $arrayOfid) {
				$arrayOfid = absint($arrayOfid);
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."origincode_contact_submission SET customer_read_or_not = '%d'  WHERE id = '%d' ", 0, $arrayOfid));
			}
		}
		return;
	}
	...
	if(isset($_POST['task'])&&$_POST['task']=='refreshSubmissions'){
		...
		if($countSub!=0){
			$submitionsCount = $wpdb->get_results("SELECT count(customer_read_or_not) AS all_count FROM " . $wpdb->prefix . "origincode_contact_submission WHERE contact_id=".$subID."");
		}
		...
		if(trim($markedSubmitions)!=''){
			if(isset($submitionsCount[0]) && $submitionsCount[0]->all_count != $counmarked && $counmarked != 0){
				$subToAppend=$wpdb->prepare("SELECT * FROM ". $wpdb->prefix . "origincode_contact_submission WHERE `id` > %d",$markedSubmitions);
				...
				foreach ($subToAppends2 as $subToAppend) {
					...
					if ($subToAppend->customer_read_or_not == 1){
						$spamer='<a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id='.$subToAppend->id.'&submissionsId='.$subToAppend->contact_id.'">'.$subToAppend->customer_country.'</a>
						<p class="spamer" '.$displayOrNot.'>Spam!</p>';
						$show='<span class="edit" value="'.$subToAppend->id.'"><a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id='.$subToAppend->id.'&submissionsId='.$subToAppend->contact_id.'">Show</a></span>';
					}else{
						$spamer='<a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id='.$subToAppend->id.'&read=unread&submissionsId='.$subToAppend->contact_id.'">'.$subToAppend->customer_country.'</a>
						<p class="spamer" '.$displayOrNot.'>Spam!</p>';
						$show='<span class="edit" value="'.$subToAppend->id.'"><a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id='.$subToAppend->id.'&read=unread&submissionsId='.$subToAppend->contact_id.'">Show</a></span>';
					}
					...
					$output.='<tr id="comment-'.$subToAppend->id.'" class="comment even thread-even '.$readOrNot.' '.$spamOrNot.' depth-'.$keyForBackground.' '.$depth.' prepended">
									...
									<td class="comment column-comment">
										...
										<div class="row-actions">'.$show.'
											...
											<span id="origincode_spinner_'.$subToAppend->id.'" class="origincode_spinner">
	...
	if(isset($_POST['task']) && $_POST['task'] == 'searchSubmission') {
		...
		if(!empty($search_value)&&$subID!='empty'){
			...
			$query = "SELECT * FROM `". $wpdb->prefix ."origincode_contact_submission` WHERE ((("
                . "" . $wpdb->prefix . "origincode_contact_submission.sub_labels LIKE '%" .$search_value . "%') OR ("
                . "" . $wpdb->prefix . "origincode_contact_submission.submission LIKE '%" .$search_value . "%') OR ("
                . "" . $wpdb->prefix . "origincode_contact_submission.submission_date LIKE '%" .$search_value . "%') OR ("
                     . $wpdb->prefix . "origincode_contact_submission.submission_ip LIKE '%" .$search_value . "%') OR ("
                     . $wpdb->prefix . "origincode_contact_submission.customer_country LIKE '%" .$search_value . "%')) AND "
                . "" . $wpdb->prefix . "origincode_contact_submission.contact_id=".$subID." )GROUP BY " . $wpdb->prefix . "origincode_contact_submission.id ORDER BY " . $wpdb->prefix . "origincode_contact_submission.id DESC";
