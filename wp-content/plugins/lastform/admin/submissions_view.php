...
function origincode_contact_sub_pagination($count2){
	//Submitions per Page
	$perpage = 50;
	...
	foreach ( $_GET as $key => $value ) {
		if ( $key != 'page_number' ) {
			$actual_link .= "{". sanitize_text_field($key) ."}=". sanitize_text_field($value) ."&amp;";
		}
...
function origincode_contact_html_showorigincode_contacts( $rows,  $pageNav,$sort,$cat_row,$form_styles){
	global $wpdb;
	?>
	...
	<div id="poststuff">
		<div id="origincode_contacts-list-page">
			<form method="post"  onkeypress="doNothing()" action="admin.php?page=origincode_forms_main_page" id="admin_form" name="admin_form">
                <h2><?php _e('All Submissions','origincode_contact');?></h2>
                <?php
			echo do_action('origincode_forms_export',$rows);
			$serch_value='';
			...
			<table class="wp-list-table widefat fixed pages" >
				<thead>
				 <tr>
					...
					<th scope="col" id="name" style="width:85px" ><span><?php _e('Submissions Form','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th scope="col" id="prod_count"  style="width:75px;" ><span><?php _e('Number of Submissions','origincode_contact');?></span><span class="sorting-indicator"></span></th>
					<th style="width:40px"><?php _e('Actions','origincode_contact');?></th>
				 </tr>
				</thead>
				<tbody>
				<?php
				...
				for ( $i = 0; $i < count( $rows ); $i ++ ) {
					...
					<tr <?php if($trcount%2==0){ echo 'class="has-background"';}?>>
						...
						<td>
                            <a  href="admin.php?page=origincode_forms_submissions&task=view_submissions&id=<?php echo $rows[$i]->id; ?>">
                                <?php echo esc_html(stripslashes($rows[$i]->name)); ?>
                            </a>
                        </td>
						<td>(<?php echo !($pr_count) ? '0' : esc_html($rows[$i]->prod_count); ?>)</td>
						<td><a  href="admin.php?page=origincode_forms_submissions&task=remove_submissions&amp;id=<?php echo esc_html($rows[$i]->id); ?>"><?php _e('Delete','origincode_contact');?></a></td>
					</tr> 
				 <?php } ?>
				</tbody>
			</table>
...
function origincode_contact_html_view_submissions($submitionsArray, $submitionsCount,$limitPage,$subName,$id) {
$keyForBackground = 1;
?>
<div class="wrap">
	<?php origincode_contact_drawFreeBanner();?>
    <div id="poststuff">
        <div id="origincode_submission_page">
            <div class="search_block">
                <form action="admin.php?page=origincode_forms_submissions" method="POST">
            ...
            <div style="clear:both;"></div> 
            <div id="origincode_top_controls">
                <ul class="controls-list">
                    ...
                    <li class="back"><a href="admin.php?page=origincode_forms_submissions" title="Back">Back</a></li>
                ...
                <?php
                	echo do_action('origincode_forms_export_single',$id);
                ...
                ?>
				    <tr id="comment-<?php echo $submition_id; ?>" class="comment even thread-even <?php if($submition->customer_read_or_not == 1){ echo "read"; } else { echo "unread"; } if($submition->customer_spam == 1){ echo " spam"; } else { echo ""; }?> depth-<?php echo $keyForBackground; if($keyForBackground%2 == 0) echo " alt"; ?> ">
				        ...
				        <td class="comment column-comment">
				            ...
				            <div class="row-actions">
				                <span class="edit" value="<?php echo $submition_id; ?>">
				                    ...
				                    <a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id=<?php echo $submition_id; ?>&submissionsId=<?php echo $submition_contact_id; ?>">Show</a>
				                    <?php else:?>
				                    <a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id=<?php echo $submition_id; ?>&read=unread&submissionsId=<?php echo $submition_contact_id; ?>">Show</a>
				                    <?php endif;?>
				                </span>
				                ...
				                <span id="origincode_spinner_<?php echo $submition_id; ?>" class="origincode_spinner">
				        ...
				        <td class="author column-author user-name">
				            <?php if ($submition->customer_read_or_not == 1):?>
							<a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id=<?php echo $submition_id; ?>&submissionsId=<?php echo $submition_contact_id; ?>"><?php echo $submition_contact_country; ?></a>
				            <p class='spamer' <?php  if($submition->customer_spam != 1) echo "style= 'display: none'"; ?>>Spam!</p>
				            <?php else:?>
				            <a href="admin.php?page=origincode_forms_submissions&task=show_submissions&id=<?php echo $submition_id; ?>&read=unread&submissionsId=<?php echo $submition->contact_id; ?>"><?php echo $submition->customer_country; ?></a>
				            <p class='spamer' <?php  if($submition->customer_spam != 1) echo "style= 'display: none'"; ?>>Spam!</p>
				            <?php endif;?>
				        </td>
			...
			<div id="origincode-contact-dialog-confirm" title="Delete Submission(s)">Are you sure?</div>
    </div>
</div>
	<script>
...
function origincode_contact_html_show_messages($messageInArray, $submitionsCount) {
	if ( isset( $_GET["id"] ) ) {
		$custom_id       = '';
...
require_once dirname(__FILE__) ."/../functions/download_file.php";
?>
<div class="wrap">
	<?php origincode_contact_drawFreeBanner();?>
    <div id="poststuff">
        <div id="origincode_single_submission_page">
            <h2>User Submission</h2>
            <div style="clear: both;"></div>
            <div id="origincode_top_controls">
                <ul class="controls-list" style="overflow: hidden;margin: 3px 0;">
                        <li class="back"><a href="admin.php?page=origincode_forms_submissions&task=view_submissions&id=<?php echo absint($messageInArray[0]->contact_id); ?>" title="Back">Back</a></li>
                        ...
                        <li class="trash" value="<?php echo esc_attr($messageInArray[0]->id); ?>" need_to_reload="yes" title="Delete"><a href="admin.php?page=origincode_forms_submissions&task=remove_submissions&amp;id=<?php echo esc_html($messageInArray[0]->id); ?>&subId=<?php echo esc_html($messageInArray[0]->contact_id); ?>">Trash</a></li>
                        <li>
                        	<img class="control_list_spinner" src="<?php echo plugins_url( '../images/spinner.gif', __FILE__ ); ?>">
                        </li>
                </ul>
            ...
            <div style="clear: both;"></div>
            <div id="origincode_messages_content">
					...
					if ( ! empty( $filesUrlExploded ) && ! empty( $filesTypeExploded ) ): ?>
						<div id="submission_attachements">
							...
							<div class="attachement_wrapper">
								<form id="my_form" action="form_ajax.php" method="post">
								<?php foreach ($filesTypeExploded as $key => $filesTypeExplodeds) {
									...
									$secure_link = wp_nonce_url($actual_link.'&file='.$file_path,'origincode_contact_donwload_file');
