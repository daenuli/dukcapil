...
function origincode_contact_get_field_row($id){
    global $wpdb;
    $id=intval($id);
    $query="SELECT * FROM  " . $wpdb->prefix . "origincode_contact_contacts_fields WHERE id={$id}";
...
function origincode_contact_create_new_captcha($captcha_id='',$from='',$time=''){

    $upload_dir=wp_upload_dir();


    if (!file_exists($upload_dir['basedir']."/origincode_forms_tmp")) {
        mkdir($upload_dir['basedir']."/origincode_forms_tmp", 0777, true);
    }

    $current_dir = getcwd(); // Save the current directory
    $dir = $upload_dir['basedir']."/origincode_forms_tmp/";
    ...
    if(isset($_POST['captchaid'])){
        $captcha_id= absint($_POST['captchaid']); $from='user';
    ...
    $field = origincode_contact_get_field_row($captcha_id);
    ...
    $_SESSION['origincode_contact_captcha-'.$from.'-'.$captcha_id.'-'.$captcha_id.$time]=$captcha;



    $font=plugin_dir_path(__FILE__).'../assets/elements/fonts/Roboto-Regular.ttf';
    ...
    if($colorOption=='default'){
        ...
        $rgbArray=origincode_hex_to_rgb($color);
    ...
    if($is_ajax_request){
        wp_send_json($upload_dir['baseurl']."/origincode_forms_tmp/".$filename);
    }

    return $upload_dir['baseurl']."/origincode_forms_tmp/".$filename;

}


function origincode_hex_to_rgb($hex) {
    $hex = str_replace("#", "", $hex);
