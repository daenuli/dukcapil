...
function  origincode_contact_html_show_emails($subscribers,$mailerParams,$count,$formsToShow,$mailing) {
?>
<div class="wrap" id="origincode_newsletter_manager">
	<?php origincode_contact_drawFreeBanner('yes');?>
	<div id="poststuff">
		<?php $path_site = plugins_url("Front_images", __FILE__); ?>
		<div id="post-body-content" class="origincode_contact_email">
            <h3>Newsletter Manager <span class="pro">PRO</span></h3>
			<div id="origincode_contact_email_manager">	
				<div class="sub_setting pro-page">
					<form method="post" action="admin.php?page=origincode_forms_email_manager&task=save" id="email_manager_form" name="email_manager_form">	
							<label for="origincode_form_choose">Choose The Forms</label><br>
							<select disabled id="origincode_form_choose">
								<option value="all">All Forms</option>
								<?php foreach ($formsToShow as $key => $value) : ?>
									<option value="<?php echo esc_html($key); ?>"><?php echo esc_html($value); ?></option>
								<?php endforeach; ?>
							</select><br>
							<label for="origincode_setting_subscriber_limit_id">Emails in One Flow</label><br>
							<input type="number" disabled id="origincode_setting_subscriber_limit_id" value="<?php echo esc_html($mailerParams['sub_count_by_parts']); ?>" class="regular-text"><br>
							<label for="origincode_setting_schedule_id">Interval Between Mailings</label><br>
							<select id="origincode_setting_schedule_id" disabled>
							...
							<label for="origincode_email_subject">Email Subject</label><br>
							<input type="text" disabled id="origincode_email_subject" value="<?php echo esc_html($mailerParams['email_subject']); ?>" class="regular-text"><br>
						<input type="hidden" name="task" value=""/>
					</form>

						<label for="wp-subscriber_message-wrap">Type your text here</label>
					<?php function origincode_contact_wptiny2( $initArray ) {
					...
					add_filter( 'tiny_mce_before_init', 'origincode_contact_wptiny2' );
					$settings = array( 'media_buttons' => false);
					wp_editor( '', "origincode_contact_subscriber_message", $settings );
					?>
					<div id="showCont">
						<?php if($mailerParams['mailing_progress']=='finish'):?>
						<div id="not_send">
							<button class="btn button-primary" id="btn" <?php if(!$subscribers) echo 'disabled'; ?>>Send <i>(pro)</i><i class="originicons-paper-plane"></i></button>
							<span id="loader" style="display: none;"><img src="<?php echo plugins_url( '../images/spinner.gif', __FILE__ ); ?>" alt=""></span>
							<span id="done" style="padding-left: 9px;display:none;">Successfully Sent <i class="originicons-check" style="color: #00A0D2;font-size: 21px;vertical-align: baseline;"></i></span>
						</div>
				...
				<div class="sub_table">
					<table class="wp-list-table widefat fixed posts" id="origincode-table">
						<thead>
							<tr>
