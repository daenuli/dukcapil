...
function origincode_contact_styles(){
	 global $wpdb;
	 
	if(isset($_GET["delete"])){
		$deleteID=absint($_GET["delete"]);
		$wpdb->query("DELETE FROM ".$wpdb->prefix."origincode_contact_style_fields  WHERE options_name = '".$deleteID."' ");
		$wpdb->query("DELETE FROM ".$wpdb->prefix."origincode_contact_styles  WHERE id = '".$deleteID."' ");
		$wpdb->query("UPDATE ".$wpdb->prefix."origincode_contact_contacts SET `hc_yourstyle` = '1' WHERE `hc_yourstyle` = '".$deleteID."'");
	}
	
	$query = "SELECT * FROM ".$wpdb->prefix."origincode_contact_styles order by id Asc";
	$rows = $wpdb->get_results($query);
	origincode_contact_html_styles($rows);
}


function origincode_contact_editstyles($op_type = "0"){
    global $wpdb;
	...
	$query="SELECT id FROM ".$wpdb->prefix."origincode_contact_styles order by id Desc";
...
('form_button_reset_icon_style', 'Form Button Reset Icon Style', 'Form Button Reset Icon Style', '".$style_id."' , 'originicons-retweet'),
...
('form_button_submit_icon_style', 'Form Button Submit Icon Style', 'Form Button Submit Icon Style', '".$style_id."' , 'originicons-rocket'),
...
('form_file_icon_style', 'Form File Icon Style', 'Form File Icon Style', '".$style_id."' , 'originicons-cloud-upload'),
...
('form_wrapper_background_color', 'Form Background Color', 'Form Background Color', '".$style_id."' , 'fcfcfc,E6E6E6')";
	...
	foreach($rowsldcc as $key=>$rowsldccs){
		if($last_key == $key){
			header('Location: admin.php?page=origincode_forms_theme_options&theme_id='.$rowsldccs->id.'');
		}
	}
	}
	...
	if ( !empty($param_values) ) {
		origincode_contact_html_editstyles( $param_values, $op_type, $style_themes );
	}
}
	...
	if ( isset( $_POST['params'] ) ) {
		if ( isset( $_POST["themeName"] ) ) {
			...
			if ( trim($_POST["themeName"]) != '' ) {
				$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "origincode_contact_styles SET  name = %s  WHERE id = %d ", $_POST["themeName"], $_GET['theme_id'] ) );
			}
		}
		$params = $_POST['params'];
		foreach ( $params as $key => $value ) {
			...
			$wpdb->query( "UPDATE " . $wpdb->prefix . "origincode_contact_style_fields SET  value = '" . $value . "'  WHERE name = '" . $key . "' and options_name = '" . $_GET['theme_id'] . "' " );
		}
		?>
