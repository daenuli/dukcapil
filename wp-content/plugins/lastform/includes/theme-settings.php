...
class Origincode_Contact_Theme_Options extends WPDEV_Settings_API_Form
{
    public $plugin_id = 'forms_contact';

    public $tablename = 'origincode_contact_style_fields';
    ...
    {
        $config = array(
            'menu_slug' => 'origincode_forms_theme_options',
            'parent_slug' => 'origincode_forms_main_page',
            'page_title' => __('Theme Options', 'origincode_contact'),
            'title' => __('Forms Contact Theme Options', 'origincode_contact'),
            'menu_title' => __('Theme Options', 'origincode_contact'),
        );
        ...
        $this->add_js ('theme-options',  plugins_url('../assets/js/theme_options.js',__FILE__));
    ...
    public function init_panels()
    {
        ...
        if(isset($_GET['page']) && $_GET['page']=='origincode_forms_theme_options' && isset($_GET['id']) && absint($_GET['id'])) {
            $themes = $wpdb->get_results('SELECT name,id FROM '.$wpdb->prefix.'origincode_contact_styles WHERE id='.absint($_GET['id']));
        } else {
            $themes = $wpdb->get_results('SELECT name,id FROM '.$wpdb->prefix.'origincode_contact_styles ORDER BY id asc');
        }

        $theme = $themes[0];

        $panelsArray[str_replace('&','_',str_replace(' ','_',strtolower($theme->name)))] = array(
            'title'=>__($theme->name,'origincode_contact'),
            'id'=>$theme->id,
            'disabled'=>true,
        ...
        foreach ($this->panels as $key=>$panel){
            if(isset($_GET['id']) && $panel['id']==$_GET['id']){
                $sectionsArray['form_block_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Form Block Styles', 'origincode_contact'),
                );
                $sectionsArray['label_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Label Styles', 'origincode_contact'),
                );
                $sectionsArray['textarea_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Textarea Styles', 'origincode_contact'),
                );
                $sectionsArray['text_input_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Text Input Styles', 'origincode_contact'),
                );
                $sectionsArray['checkbox_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Checkbox Styles', 'origincode_contact'),
                );
                $sectionsArray['selectbox_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Selectbox Styles', 'origincode_contact'),
                );
                $sectionsArray['radio_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Radio Styles', 'origincode_contact'),
                );
                $sectionsArray['pagination_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Pagination Styles', 'origincode_contact'),
                );
                $sectionsArray['file_upload_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('File Uploader Styles', 'origincode_contact'),
                );
                $sectionsArray['button_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Button Styles', 'origincode_contact'),
                );
                $sectionsArray['custom_styles'.$panel['id']] = array(
                    'panel' => $key,
                    'title' => __('Custom Styles', 'origincode_contact'),
                );
            }
        }
    ...
    private function init_theme_options()
    {
        foreach ($this->panels as $key=>$panel){
            if(isset($_GET['id']) && $panel['id']==$_GET['id']){
                ...
                $this->{'form_file_icon_style'.$panel['id']} = $this->get_option_from_table("form_file_icon_style", 'originicons-paperclip', true, array('options_name'=>$panel['id']));
                ...
                $this->{'form_button_submit_icon_style'.$panel['id']} = $this->get_option_from_table("form_button_submit_icon_style", 'originicons-mail-forward', true, array('options_name'=>$panel['id']));
                ...
                $this->{'form_button_reset_icon_style'.$panel['id']} = $this->get_option_from_table("form_button_reset_icon_style", 'originicons-refresh', true, array('options_name'=>$panel['id']));
        ...
        foreach($this->panels as $key=>$panel){
            if(isset($_GET['id']) && $panel['id']==$_GET['id']){
                ...
                $controlsArray['form_wrapper_width'.$panel['id']] = array(
                    ...
                    'label' => __('Form Width', 'origincode_contact'),
                    'help' => __('Select the width of the form by dragging the circle.', 'origincode_contact')
                );
                $controlsArray['form_wrapper_background_type'.$panel['id']] = array(
                    ...
                    'label' => __('Form Background Type', 'origincode_contact'),
                );
                ...
                $controlsArray['form_show_title'.$panel['id']] = array(
                    ...
                    'label' => __('Form Show Title', 'origincode_contact'),
                );
                $controlsArray['form_title_size'.$panel['id']] = array(
                    'section' => 'form_block_styles'.$panel['id'],
                    'type' => 'number',
                    'default' =>  $this->{'form_title_size'.$panel['id']},
                    'label' => __('Form Title Size (px)', 'origincode_contact'),
                    'help' => __('Select the font size of the form title.', 'origincode_contact')
                );
                $controlsArray['form_title_color'.$panel['id']] = array(
                    'section' => 'form_block_styles'.$panel['id'],
                    'type' => 'color',
                    'default' =>  $this->{'form_title_color'.$panel['id']},
                    'label' => __('Form Title Color', 'origincode_contact'),
                    'help' => __('Select the text color of the form title.', 'origincode_contact')
                );
                ...
                $controlsArray['form_label_size'.$panel['id']] = array(
                    ...
                    'label' => __('Label Size', 'origincode_contact'),
                    'help' => __('The label font size can be editted here.', 'origincode_contact')
                );
                ...
                $controlsArray['form_label_success_message'.$panel['id']] = array(
                    ...
                    'label' => __('Success Message Color', 'origincode_contact'),
                    'help' => __('Success Message is displayed below the form after successful submission.', 'origincode_contact')
                );
                ...
                $controlsArray['form_checkbox_size'.$panel['id']] = array(
                    ...
                    'label' => __('Checkbox Size', 'origincode_contact'),
                );
                ...
                $controlsArray['form_radio_size'.$panel['id']] = array(
                    ...
                    'label' => __('Radio Size', 'origincode_contact'),
                );
                ...
                $controlsArray['form_file_has_icon'.$panel['id']] = array(
                    ...
                    'label' => __('Upload Button Has Icon', 'origincode_contact'),
                );
                $controlsArray['form_file_icon_style'.$panel['id']] = array(
                    ...
                    'height' => 30,
                    'choices' => array(
                        "originicons-paperclip",
                        "originicons-camera",
                        "originicons-picture-o",
                        "originicons-file",
                        "originicons-dropbox",
                        "originicons-cloud",
                        "originicons-cloud-upload",
                        "originicons-download",
                        "originicons-cloud-download",
                        "originicons-file-pdf-o",
                        "originicons-file-text",
                        "originicons-file-excel-o",
                        "originicons-file-powerpoint-o",
                        "originicons-file-zip-o",
                        "originicons-file-audio-o",
                        "originicons-floppy-o",
                        "originicons-music",
                        "originicons-film",
                        "originicons-camera-retro",
                        "originicons-gift"
                    ),
                    'default' => $this->{'form_file_icon_style'.$panel['id']},
                    'label' => __('Upload Button Icon', 'origincode_contact'),
                );
                ...
                $controlsArray['form_button_position'.$panel['id']] = array(
                    ...
                    'label' => __('Submit/Reset Buttons Alignment', 'origincode_contact'),
                );
                ...
                $controlsArray['form_button_icons_position'.$panel['id']] = array(
                    ...
                    'label' => __('Icons Position', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_has_icon'.$panel['id']] = array(
                    ...
                    'label' => __('Submit Button Has Icon', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_icon_style'.$panel['id']] = array(
                    'section' => 'button_styles'.$panel['id'],
                    'type' => 'icon_radio',
                    'choices'=>array(
                        "originicons-mail-forward",
                        "originicons-mail-reply",
                        "originicons-clock",
                        "originicons-bell",
                        "originicons-paper-plane",
                        "originicons-sign-in",
                        "originicons-bars",
                        "originicons-child",
                        "originicons-gift",
                        ...
                        "originicons-anchor",
                        "originicons-plus",
                        "originicons-envelope-o",
                        "originicons-envelope",
                        "originicons-cart-plus"
                    ),
                    'default' => $this->{'form_button_submit_icon_style'.$panel['id']},
                    'label' => __('Submit Button Icon Image', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_icon_color'.$panel['id']] = array(
                    'section' => 'button_styles'.$panel['id'],
                    'type' => 'color',
                    'default' => $this->{'form_button_submit_icon_color'.$panel['id']},
                    'label' => __('Submit Button Icon Color', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_icon_hover_color'.$panel['id']] = array(
                    ...
                    'label' => __('Submit Button Hover Icon Color', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_font_color'.$panel['id']] = array(
                    'section' => 'button_styles'.$panel['id'],
                    'type' => 'color',
                    'default' => $this->{'form_button_submit_font_color'.$panel['id']},
                    'label' => __('Submit Button Font Color', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_font_hover_color'.$panel['id']] = array(
                    ...
                    'label' => __('Submit Button Hover Font Color', 'origincode_contact'),
                );
                $controlsArray['form_button_submit_background'.$panel['id']] = array(
                    'section' => 'button_styles'.$panel['id'],
                    'type' => 'color',
                    'default' => $this->{'form_button_submit_background'.$panel['id']},
                    'label' => __('Submit Button Background Color', 'origincode_contact'),
                );
                ...
                $controlsArray['form_button_reset_has_icon'.$panel['id']] = array(
                    ...
                    'label' => __('Reset Button Has Icon', 'origincode_contact'),
                );
    ...
    public function init_admin() {
        if(isset($_GET['task']) && $_GET['task']=='edit_theme'){
            ...
            global $wpdb;
            $themes = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'origincode_contact_styles');
    ...
    public function update_theme_title(){
        ...
        $theme_title = sanitize_html_class($_REQUEST['title']);

        if($theme_id && $theme_title){
            global $wpdb;
            $result = $wpdb->update($wpdb->prefix.'origincode_contact_styles',array('name'=>$theme_title),array('id'=>$theme_id));
                    ...
                    <form method="post"  onkeypress="doNothing()" action="admin.php?page=origincode_forms_main_page" id="admin_form" name="admin_form">
                        <h2>
                            ...
                            <a onclick="alert('This option is disabled for free version. Please upgrade to pro license to be able to use it.');	" class="add-new-h2" >
                                <?php _e('Add New Theme','origincode_contact');?>
                            </a>
                        </h2>
                    ...
                    <div class="alignleft actions">
                        ...
                        <table class="wp-list-table widefat fixed pages">
                            ...
                            <tr>
                                ...
                                <th scope="col" id="name" style="width:85px" ><span><?php _e('Theme Name', 'origincode_contact'); ?></span><span class="sorting-indicator"></span></th>
                            ...
                            <?php foreach ($themes as $theme):?>
                                <tr>
                                    ...
                                    <td ><span><a href="admin.php?page=origincode_forms_theme_options&task=edit_theme&id=<?php echo intval($theme->id);?>"><?php echo $theme->name;?></span></a></td>
                                    <td ><span><?php echo $theme->last_update;?></span></td>
                                    <td ><a  onclick="alert('This option is disabled for free version. Please upgrade to pro license to be able to use it.');	" > <i style="font-size: 18px; margin-left: 14px" class="originicons-trash"></i> </a></td>
                                </tr>
                            <?php endforeach;?>
                            </tbody>
                        </table>
