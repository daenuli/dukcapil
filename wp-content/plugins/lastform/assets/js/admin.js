...
jQuery(document).ready(function () {
	...
	jQuery(".origincode_contact_custom_settings_dropdown_heading").on("click",function () {
		jQuery(".origincode_contact_custom_settings_dropdown_content").toggleClass("-hidden");
		if( jQuery(".origincode_contact_custom_settings_dropdown_heading i").hasClass("originicons-chevron-down") ){
			jQuery(".origincode_contact_custom_settings_dropdown_heading i")
				.removeClass("originicons-chevron-down")
				.addClass("originicons-chevron-up");
		}else{
			jQuery(".origincode_contact_custom_settings_dropdown_heading i")
				.removeClass("originicons-chevron-up")
				.addClass("originicons-chevron-down");
		}
	});

	if( jQuery("#origincode_contact_user_message").length ){
		jQuery("#origincode_contact_user_message").attr("disabled","disabled");
	}

	if( jQuery("#origincode_contact_admin_message").length ){
		jQuery("#origincode_contact_admin_message").attr("disabled","disabled");
	}

	/*******//////////////////Submission Scripts////////////////*********/
	//   CHECK OR UNCHECK ALL SUBMITIONS
	var check_all = "#origincode_submission_page #origincode_top_controls .select input[name='all']";
	...
	// Check READ/UNREAD
	jQuery('#origincode_submission_page #origincode_top_controls .controls-list .select select').change(function(){   //   alert(jQuery(this).val());
	...
	jQuery('#origincode_submission_page #origincode_top_controls .controls-list li').click(function(e){
		...
		var self=jQuery('#origincode_submission_page #origincode_top_controls .controls-list li');
		...
		if(marked_submitions.length > 0){                         // IF EXIST SOME CHECKED SUBMITION
			if(command == "spam"){                                 // IF CLICKED IN SPAM IMAGE
				var data = {
					action: 'origincode_contact_action',
				...
				forEach( marked_submitions, function( submition_id ) {
					function showRowActions(){
						...
						jQuery('#comment-'+submition_id+'').find('#origincode_spinner_'+submition_id+' img').fadeIn();
					}
					showRowActions();
				})
				jQuery.post(ajaxurl, data, function(response) {   //    alert(response);
					if(response) {                                //    alert(reviews_for_delete);
						...
						forEach( marked_submitions, function( submition_id ) {    //    alert( submition_id );
							...
							jQuery('#comment-'+submition_id+'').find('#origincode_spinner_'+submition_id+' img').fadeOut();
				...
				if(command == "trash"){                             // IF CLICKED IN TRASH IMAGE
					jQuery( "#origincode-contact-dialog-confirm" ).dialog({ // ALERTING ARE YOU SURE DIALOG
		...
		if(command == "refrash"){
			...
			var countTorefresh=jQuery('#origincode_submission_page input[name="countTorefresh"]').val();
			var subID=jQuery('#origincode_submission_page input[name="subID"]').val();
			var marked_submitions_refresh= jQuery("#origincode_submission_page").find("input[name='check_comments']").filter(':first').val();
			jQuery.ajax({
				type: "POST",
				url:ajaxurl,
				data:{
				...
				success: function(response){
					...
					if(response.output){
						...
						jQuery("#origincode_submission_page table").find("tbody").prepend(response.output);
						self.parent().find('li img.control_list_spinner').fadeOut();
						setTimeout(function(){
							jQuery("#origincode_submission_page table").find("tbody").find(".prepended").removeClass("prepended");
						},1000);
					}else{
						self.parent().find('li img.control_list_spinner').fadeOut();
					}
	...
	jQuery('#origincode_submission_page').on('click tap','.row-actions .not_spam a',function(){
		...
		jQuery.ajax({
			...
			beforeSend:function(){
				...
				self.parent().parent().css('display','table-row');
				self.parent().parent().find('#origincode_spinner_'+submissionId+' img').fadeIn();
			},
			success: function(response){
				if(response) {
					...
					self.parent().parent().find('#origincode_spinner_'+submissionId+' img').fadeOut();
	...
	//////////////Mark As Spam Single////////////////////////
	jQuery('#origincode_submission_page').on('click tap','.row-actions .spam a',function(){
		var self=jQuery(this);
		var submissionId=jQuery(this).parent().attr('value');
		jQuery.ajax({
			type: "POST",
	...
	jQuery('#origincode_single_submission_page #origincode_top_controls li.spam a').click(function(){
		var self=jQuery(this);
		var submissionId=jQuery(this).parent().attr('value');
	...
	/////////////Delete Single//////////////////////////
	jQuery('#origincode_submission_page').on('click tap','.row-actions .trash a',function(){
		var self=jQuery(this);
		var submissionId=jQuery(this).parent().attr('value');
		jQuery( "#origincode-contact-dialog-confirm" ).dialog({ // ALERTING ARE YOU SURE DIALOG
			dialogClass:'dialog_style56',
	...
	//////Search
	jQuery('#origincode_submission_page > .search_block .button').click(function(e){
		e.preventDefault();
		var self=jQuery(this);
		var subID=jQuery('#origincode_submission_page input[name="subID"]').val();
		...
		if(searchData==''){
			...
			jQuery.ajax({
				...
				success: function(response){
					var response = jQuery.parseJSON(response);
					if(response.output) {
						jQuery("#origincode_submission_page table").find("tbody").html(response.output);
	...
	/*******////////BULK ACTIONS////////*********/
	jQuery('#origincode_submission_page #origincode_top_controls .controls-list li a.apply').click(function(e){
		e.preventDefault();
		...
		if(selectVal=='none'){
			...
			if(marked_submitions.length > 0){
				...
				jQuery('#origincode_top_controls .controls-list li img.control_list_spinner').fadeIn();
				var forEach = Function.prototype.call.bind( Array.prototype.forEach );
				forEach( marked_submitions, function( submition_id ) {
					function showRowActions(){
						jQuery('#comment-'+submition_id+'').hover(function(){
				...
				jQuery.post(ajaxurl, data, function(response) {   //    alert(response);
					if(response) {                                //    alert(reviews_for_delete);
						...
						jQuery('#origincode_top_controls .controls-list li img.control_list_spinner').fadeOut();
					}
				});
			}
	...
	jQuery('#origc_n_btn_block').on('click','input#save-buttom',function(){
		form_clean = jQuery("form").serialize();
	...
	if(pageCheck != 'origincode_forms_theme_options' && pageCheck != 'origincode_forms_general_options'){
	...
	jQuery('.fields-list ').on('click tap','li > div .open-close',function(){
		...
		if(jQuery(this).parent().parent().parent().hasClass('open')){
			...
			jQuery('.origincode_contact_custom_settings_main').animate({top: 0 + 'px'});
		}else {
			var height_1, height_2, height_3;
			setTimeout(function(){
				height_1 = +(jQuery('.fields-list > li.open').height());
				height_2 = +(jQuery('#origincode-contact-preview-container').height());
				if(height_2 < height_1){
					height_3 = Math.max(height_1 - height_2, height_2 - height_1);
				} else {
					height_3 = 0;
				}
				jQuery('.origincode_contact_custom_settings_main').animate({top:  height_3 + 'px'});
			}, 100);

            jQuery('a.add-new ').on('click',function(){
                height_4 = +(jQuery('.fields-list > li.open').height());
                jQuery('.origincode_contact_custom_settings_main').animate({top:  height_4 - height_1 + 40 + 'px'});
            });
	...
	jQuery("#fields-list-block").on('click','.fields-list .field-multiple-option-list li .set-active input',function(){
		...
		if(jQuery(this).parents('.field-multiple-option-list').hasClass('checkbox')){
			if(jQuery(this).parent().hasClass('checked')){
				...
				jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] ul').find('li').eq(index).find('input[type="checkbox"]').removeAttr('checked');
			}else {
				jQuery(this).parent().addClass("checked");
				var previewcheckbox=jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] ul').find('li').eq(index).find('input[type="checkbox"]');
		...
		/*selectbox*/
            ...
            jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] select').find('option').removeAttr('selected');
            var previewselect = jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] select');
            ...
            jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] .textholder').val( previewselect.val() );
		...
		/* radio */
			...
			jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] ul').find('li input[type="radio"]').removeAttr('checked');
			var previewradio=jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] ul').find('li').eq(index).find('input[type="radio"]');
	...
	jQuery("#fields-list-block").on('keypress keyup change','.fields-list .field-multiple-option-list li input:text',function(){
		if(!jQuery(this).hasClass('add-new-name')){
			...
			if(jQuery(this).parents('.field-multiple-option-list').hasClass('selectbox')){
				jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] select').find('option').eq(index).html(jQuery(this).val());
				if(index==jQuery(this).parents(".field-multiple-option-list").find('.field-multiple-option-active-field').val()){
					jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] .textholder').val(valToChange);
				}
			}else{
				jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-'+fieldID+'"] ul').find('li').eq(index).find('.sublable').html(jQuery(this).val());
			}
			var allvalues='';
	...
	jQuery("#fields-list-block").on('click','.fields-list .field-multiple-option-list li .add-new',function(){
		...
		if(jQuery(this).parents(".field-multiple-option-list").hasClass('selectbox')){
			...
			var previewradio = jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"]').find('.field-block input').last();
	...
	jQuery("#fields-list-block").on('click', '.fields-list .field-multiple-option-list li .remove-field-option', function() {
		...
		if (elemCount != 2) {
            var $top = jQuery('.origincode_contact_custom_settings_main').css('top');

            jQuery('.origincode_contact_custom_settings_main').animate({top:  parseInt($top) - 30 + 'px'});
			...
			if (jQuery(this).parents('.field-multiple-option-list').hasClass('selectbox')) {
				...
				if (allowChange != 0) {
					...
					if (jQuery(this).parent().index() == 0) {
						...
						jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] .textholder').val(secondval);
					} else {
						jQuery(this).parents('.field-multiple-option-list').find('li').eq(0).find('.set-active').addClass('checked');
						jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] .textholder').val(firstval);
					}
					jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] select').find('option').eq(index).remove();
				}
				...
				if (jQuery(this).parent().index() == 0) {
					jQuery(this).parents('.field-multiple-option-list').find('li').eq(1).find('.set-active').addClass('checked');
					var previewradio = jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] ul').find('li').eq(1).find('input[type="radio"]');
					previewradio.attr('checked', 'checked');
				} else {
					jQuery(this).parents('.field-multiple-option-list').find('li').eq(0).find('.set-active').addClass('checked');
					var previewradio = jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] ul').find('li').eq(0).find('input[type="radio"]');
					previewradio.attr('checked', 'checked');
				}
				jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] ul').find('li').eq(index).remove();
			} else {
				jQuery('.origincode-contact-column-block > div[rel="origin-contact-field-' + fieldID + '"] ul').find('li').eq(index).remove();
			}
			allvalues = allvalues.slice(0, -2);
    ...
    jQuery("#fields-list-block").on('change keyup','.fields-list li[data-fieldtype=simple_captcha_box] input[type=number],.fields-list li[data-fieldtype=simple_captcha_box] input.color',function(){
        ...
        if(digits){
            ...
            if(jQuery('.origincode-field-block.simple-captcha-block label>img').length){
                jQuery('.origincode-field-block.simple-captcha-block label>img').remove();
                jQuery('.origincode-field-block.simple-captcha-block label').prepend('<div class="simple-captcha-rect" style="display:inline-block; text-align:center; line-height: 60px; font-size:30px;height:60px; width: 170px; color: #fff; background-color: #' + bgcolor + ' ">'+text+'</div>')
            } else {
                jQuery('.origincode-field-block.simple-captcha-block label .simple-captcha-rect').text(text).css('background-color','#'+bgcolor);
    ...
    jQuery("#fields-list-block").on('change keyup', '.fields-list .fields-options .field-columns-count', function() {
		...
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] ul li').css({'width': liwidth + '%'});
	});


	/*##########ONCHANGE############*/
	jQuery('.origincode_contact_top_tabs>li').on('keyup', 'input.text_area', function() {
		var titleVal = jQuery(this).val();
		jQuery('.text_area_title').val(titleVal);
	});
	jQuery('.text_area_title').on('keyup', function() {
		var titleVal = jQuery(this).val();
		jQuery('.origincode_contact_top_tabs input.text_area').val(titleVal);
	});
	jQuery('.origincodeTitleOverlay').click(function() {
	...
	jQuery('#fields-list-block > ul').on({
		mouseenter: function() {
			...
			jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"]').addClass('hover-active');
			jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"]').find('span.originOverlay').css('display', 'block');
		},
		mouseleave: function() {
			var fieldid = jQuery(this).attr('id');
			jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"]').removeClass('hover-active');
			jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"]').find('span.originOverlay').css('display', 'none');
		}
	}, "li");
	jQuery('#origincode-contact-wrapper .origincode-contact-column-block').on({
		mouseenter: function() {
			var fieldid = jQuery(this).attr('rel');
			jQuery(this).find('span.originOverlay').css('display', 'block')
		...
		mouseleave: function() {
			...
			jQuery(this).find('span.originOverlay').css('display', 'none')
		}
	}, "div.origincode-field-block");
	...
	jQuery('#fields-list-block').on('keyup change', 'input.label', function() {
		if (jQuery(this).parents('.fields-options').find('select#form_label_position').val() == 'formsInsideAlign') {
			...
			var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
			var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block input');
			var previewfieldtextarea = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block textarea');
		...
		var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] label').not('.secondary-label');
		var addstar = "";
	...
	jQuery('#fields-list-block').on('keypress keyup change', '.fieldisactive', function() {
            var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
            var previewfieldtextarea = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] textarea');
            var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block input');
            if (jQuery(this).is(':checked')) {
    ...
    jQuery('#fields-list-block').on('keypress keyup change', 'input.placeholder-option', function() {
        ...
        if( _this ){
            if(jQuery('#origincode_preview_textbox_'+fieldid+' option.placeholder-option').length){
                jQuery('#origincode_preview_textbox_'+fieldid+' option.placeholder-option').text(placeholder);
            } else {
                jQuery('#origincode_preview_textbox_'+fieldid).prepend('<option class="placeholder-option" selected="selected">'+placeholder+'</option>');
            }

            jQuery('.origincode-field-block[rel=origin-contact-field-'+fieldid+'] .textholder').val(placeholder);
            ...
            jQuery('#origincode_preview_textbox_'+fieldid+' option.placeholder-option').remove();
            var firstOption = jQuery('#origincode_preview_textbox_'+fieldid +' option:first-child').text();
            jQuery('.origincode-field-block[rel=origin-contact-field-'+fieldid+'] .textholder').val(firstOption);
	...
	jQuery('#fields-list-block').on('change keyup', '.textbox_file_type input', function() {
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block > input').prop('type', value);

		if(value=='number'){
            jQuery('#' + fieldid + ' .origc-mask-on-check').attr('disabled',true).attr('checked', false);
		} else {
            jQuery('#' + fieldid + ' .origc-mask-on-check').attr('disabled', false);
		}
	});
	...
	jQuery('#fields-list-block').on('change keyup', '.textarea-resize', function() {
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var textarea = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] textarea').not('.secondary-label');
		if (jQuery(this).is(':checked')) {
	...
	jQuery('#fields-list-block').on('change keyup', '.textarea-size', function() {
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var textarea = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] textarea');
	...
	jQuery('#fields-list-block').on('keypress keyup', '.submitbutton', function() {
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var submitbutton = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] button[type="submit"]');
	...
	jQuery('#fields-list-block').on('keypress', '.resetbutton', function() {
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var resetbutton = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] button[type="reset"]');
	...
	jQuery('#fields-list-block').on('change', '.origincode_contact_captcha_theme', function() {
	...
	jQuery('#fields-list-block').on('change', '.captcha_position', function() {
		if (jQuery('#fields-list-block .origincode_contact_captcha_theme').val() == 'light') {
	...
	jQuery('#fields-list-block').on('keypress keyup change', 'select#ready_form_label_position', function() {
		...
		var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] label');
		var previewfield2 = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] div.field-block');
	...
	jQuery('#fields-list-block').on('keypress keyup change', '.placeholderName', function() {
		...
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block input.pl_name');
	...
	jQuery('#fields-list-block').on('keypress keyup change', '.placeholderSur', function() {
		...
		var value = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var previewfield = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block input.pl_surname');
		previewfield.attr("placeholder", value);
	});
	jQuery('#fields-list-block').on('keypress keyup change', 'select.country-list', function() {
		...
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		var numCode = jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block ul.country-list').find('li[data-country-code="' + codeName + '"]').attr('data-dial-code');
		numCode = '+' + numCode;
		...
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block div.selected-flag').click();
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .field-block ul.country-list').find('li[data-country-code="' + codeName + '"]').click();
	});
	jQuery('#fields-list-block').on('keypress keyup change', 'input.linkName', function() {

		var codeName = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .license-block').find('span.sublable a').text(codeName);
	});
	jQuery('#fields-list-block').on('keypress keyup change', 'input.linkUrl', function() {

		var url = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .license-block').find('span.sublable a').attr('href', url);
	});
	jQuery('#fields-list-block').on('keypress keyup change', 'textarea.fieldContent', function() {
		...
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .license-block').find('span.sublable').html(fieldContent);
	});
	jQuery('#fields-list-block').on('keypress keyup change', 'select.fieldPos', function() {
		var position = jQuery(this).val();
		var fieldid = jQuery(this).parents('.fields-options').parent().attr('id');
		jQuery('.origincode-contact-column-block > div[rel="' + fieldid + '"] .license-block').css('text-align', position)
	});
	//Ready to Go Onchange//
	/*edit field*/
	jQuery('#origincode-contact-wrapper').on('click tap', '.origincode-field-block>label', function(e) {
		e.preventDefault();
	});
	jQuery('#origincode-contact-wrapper').on('click tap', '.origincode-field-block', function() {
		var fieldID = jQuery(this).attr('rel');
		jQuery('#fields-list-block').find('li#' + fieldID + ' .field-top-options-block a.open-close').click();
	});
	jQuery('#origincode-contact-wrapper').on('click tap', '.origincode-field-block .flag-container', function() {
		return false;
	});
	/*edit field*/
	/*FRONT END PREVIEW */

	jQuery(".origincode-contact-column-block input[type='file']").on('change keyup', function() {
		var value = jQuery(this).val().substr(jQuery(this).val().indexOf('fakepath') + 9);
		jQuery(this).parent().find('input[type="text"]').val(value);
	});

	jQuery(".origincode-contact-column-block select").on('change keyup', function() {
	...
	jQuery('.origincode_forms_delete_form').on('click', function() {
	...
	jQuery('.origincode_contact_duplicate_form').on('click', function(e) {
		e.preventDefault();
		...
		jQuery.ajax({
			...
			type: 'POST',
			data: {
				action: 'origincode_contact_duplicate_form',
				nonce: nonce,
				id: id
			}
	...
	jQuery(".close_free_banner").on("click",function(){
		...
		OrigincodeContactSetCookie( 'origcFormsFreeBannerShow', 'no', {expires:86400} );
	});
	defaultTitleVisibility = jQuery('#origincode-contact-wrapper').find('h3').css('display');

	jQuery('#select_form_show_title').on('change', function() {
		switch (jQuery(this).val()) {
			case 'yes' :
				jQuery('#origincode-contact-wrapper').find('h3').css('display', 'block');
				break;
			case 'no' :
				jQuery('#origincode-contact-wrapper').find('h3').css('display', 'none');
				break;
			case 'default' :
				jQuery('#origincode-contact-wrapper').find('h3').css('display', defaultTitleVisibility);
		}
	});
...
function OrigincodeContactSetCookie(name, value, options) {
	options = options || {};
...
var checkAnimate;
jQuery(function() {
	jQuery(".origincode-contact-column-block").sortable({
		...
		start: function(e, ui) {
			...
			if (jQuery("#fields-list-right li").length > 0) {
				...
				var hieghtOfColumn = jQuery("#origincode-contact-wrapper  div.origincode-contact-block-left").height();
				var res = (hieghtOfColumn - 116) / 2;
				jQuery("#origincode-contact-wrapper  div.origincode-contact-block-left").animate({'width': "70%"}, 500);
				jQuery("#origincode-contact-wrapper  div.origincode-contact-block-right").animate({
					'width': "23%",
					'min-width': "23%"
				}, 500, function() {
		...
		connectWith: ".origincode-contact-column-block",
		stop: function(e, ui) {
			var leftelement = [];
			var rightelement = [];
			jQuery(".origincode-contact-block-left > div").each(function() {
				leftelement.push(jQuery('.fields-list li[id="' + jQuery(this).attr('rel') + '"]'));
				jQuery(this).find('.left-right-position').val('left');
			});

			jQuery(".origincode-contact-block-right > div").each(function() {
			...
			var j = 0;
			jQuery(".origincode-contact-block-left > div.origincode-field-block").each(function() {
				jQuery(this).find('input.ordering').val(j);
				j++;
			});

			var k = 0;
			jQuery(".origincode-contact-block-right > div.origincode-field-block").each(function() {
				jQuery(this).find('input.ordering').val(k);
				k++;
			});

			if (jQuery("#origincode-contact-wrapper  div.origincode-contact-block-right>div").length > 0) {
				if (jQuery("#origincode-contact-wrapper  div.origincode-contact-block-right>div").length == 1 && checkAnimate == false) {
					jQuery("#origincode-contact-wrapper  div.origincode-contact-block-left").animate({'width': "47%"}, 500);
					jQuery("#origincode-contact-wrapper  div.origincode-contact-block-right").animate({'width': "47%"}, 500);
				} else {
					checkAnimate = true;
				}
			} else {
				jQuery("#origincode-contact-wrapper  div.origincode-contact-block-left").animate({'width': "93%"}, 500);
				jQuery("#origincode-contact-wrapper  div.origincode-contact-block-right").animate({
					'width': "1%",
					"min-width": "1%"
				}, 500, function() {
			...
			if (jQuery("#fields-list-right li").length > 0) {
				jQuery("#origincode-contact-wrapper > div").addClass('multicolumn');
			} else {
				jQuery("#origincode-contact-wrapper > div").removeClass('multicolumn');
			}
	...
	jQuery('.origincode-contact-column-block').on('click', 'input', function() {
		jQuery(this).focus();
	});
	jQuery('.origincode-contact-column-block').on('click', 'textarea', function() {
		jQuery(this).focus();
	});
	jQuery('.origincode-contact-column-block').on('click', 'select', function() {
...
jQuery( ".origc_view_plugins_block .toggle_element" ).toggle(function() {
    ...
    if (screen < 415) {
        jQuery('.origincode_logo').addClass('hide');
	...
	if(jQuery(document).find("#origincode_contact_formname").length){
        var _fn_width = jQuery("#origincode_contact_formname").val().length;
        jQuery("#origincode_contact_formname").width((_fn_width+2)*8+"px");
	}

/*Mask On*/
	(function(){
		 var def_value;
        jQuery('#fields-list-block').on("change",".origc-mask-on-check",function() {
				  var mask_on_block = jQuery(this).parent().find('.origc-mask-on');
				  def_value   = mask_on_block.closest('div.fields-options').find('.origc-def-value');
