...
(function($) {
    ...
    origcform_igw_i18n = {'origcform'	:'OriginCode Forms', 'edit':'Click to edit', 'remove':	'Click to remove'};

    tinyMCE.addI18n('en.origcform_attach_to_post', {
        title: 'Attach OriginCode Form to Post'
    });

    // Create the plugin. We'll register it afterwards
    tinymce.create('tinymce.plugins.ORIGCForms_Attach_To_Post', {
        /**
         * Returns metadata about this plugin
         */
        getInfo: function() {
            return {
                longname: 'OriginCode Forms',
                author: 'Imagely',
        ...
        init: function(editor, plugin_url) {
            ...
            editor.windowManager.origcformOldOpen = editor.windowManager.open;
            editor.windowManager.open = function(one, two) {
                var modal = editor.windowManager.origcformOldOpen(one, two);
                modal.on('close', self.wm_close_event);
                return modal;
            };

            // Register a new TinyMCE command
            editor.addCommand('origcform_attach_to_post', this.render_attach_to_post_interface, {
                editor: editor,
                plugin:	editor.plugins.ORIGCForms_Attach_To_Post
            });

            // Add a button to trigger the above command
            editor.addButton('ORIGCForms_Attach_To_Post', {
                title:	'origcform_attach_to_post.title',
                cmd:	'origcform_attach_to_post',
            ...
            editor.on('mouseup', function(e) {
                ...
                // Support for IGW Visual Shortcodes. NGG >= 2.1.50.1
                    ...
                    var $target = $(e.target);
                    if ($target.parents('.origcformsPlaceholderButton')) {
                        $target = $target.parents('.origcformsPlaceholderButton');
                    }
                    if ($target.hasClass('origcformsPlaceholderButton')) {

                        // Remove button
                        if ($target.hasClass('origcformsPlaceholderRemove')) {
                            $target.parents('.origcformsPlaceholder').remove();
                        }

                        // Edit button
                        else {
                            window.igw_shortcode=  $(e.target).parents('.origcformsPlaceholder').data('shortcode');
                            self.render_attach_to_post_interface({
                                key: 'shortcode',
                                val: Base64.encode(window.igw_shortcode),
                                ref: $(e.target).parents('.origcformsPlaceholder').attr('id')
                            });
                        }
                    }
                }
            ...
            editor.on('BeforeSetContent', function(event){
                var shortcode_opening_tag = '[origincode_forms';
                while (event.content.indexOf(shortcode_opening_tag)>=0) {
                    ...
                    var template = _.template($('#origincode-shortcode-placeholder').html());
                    event.content = event.content.replace(match, template($.extend(origcform_igw_i18n, {
                        shortcode: shortcode,
                        ref: _.now()
                    })));
                }
            ...
            editor.on('PostProcess', function(event) {
                ...
                $content.find('.origcformsPlaceholder').toArray().forEach(function(placeholder){
        ...
        wm_close_event: function(e) {
            if (e && e.target && e.target._id && e.target._id == 'origcform_attach_to_post_dialog') {
        ...
        render_attach_to_post_interface: function(params) {
            ...
            var attach_to_post_url = origcform_attach_to_post_url;
            ...
            this.editor.windowManager.open({
                ...
                id: 'origcform_attach_to_post_dialog',
                width: popupWidth,
                height: popupHeight,
                title: "OriginCode Forms - Attach To Post"
            });
            ...
            $('#origcform_attach_to_post_dialog_ifr').css('overflow-y', 'auto');
            $('#origcform_attach_to_post_dialog_ifr').css('overflow-x', 'hidden');
        }
    });

    // Register plugin
    tinymce.PluginManager.add('ORIGCForms_Attach_To_Post', tinymce.plugins.ORIGCForms_Attach_To_Post);
