jQuery(document).ready(function(e) {
        ...
        if (jQuery('#add-fields-block li.spinnerLi>img').css('display') != 'inline') {
            if (inputTypeStatus != 'disabled' && inputType != 'Nocaptcha' && inputType != 'custom_text') {
                ...
                jQuery.ajax({
                    ...
                    url: ajaxurl,
                    data: {
                        action: 'origincode_contact_formBuilder_action',
                        task: 'addFieldsTask',
                        formId: formId,
                    ...
                    if (response) {
                        if (response.captchaNum) {
                            ...
                            if (jQuery('#origincode-contact-wrapper .origincode-contact-block-right div').length == 0) {
                                ...
                                if (ifExists) {
                                    var beforeAdd = jQuery('.origincode-contact-block-left').find('div.buttons-block');
                                    jQuery(response.outputField).insertBefore(beforeAdd);
                                } else {
                                    jQuery('#origincode-contact-wrapper .origincode-contact-block-left').append(response.outputField);
                                }
                            } else {
                                ...
                                if (ifExists) {
                                    var beforeAdd = jQuery('.origincode-contact-block-right').find('div.buttons-block');
                                    jQuery(response.outputField).insertBefore(beforeAdd);
                                } else {
                                    jQuery('#origincode-contact-wrapper .origincode-contact-block-right').append(response.outputField);
                                }
                            }
                            var leftelement = [];
                            var rightelement = [];
                            ...
                            jQuery('#origincode-contact-wrapper .origincode-contact-block-left').prepend(response.outputField);
                            ...
                            if( inputType == 'simple_captcha_box'){
                                if( jQuery('li[data-fieldtype=buttons]').length ){
                                    ...
                                    jQuery('.origincode-field-block.buttons-block').before(response.outputField);
                                } else {
                                    jQuery('#origincode-contact-wrapper .origincode-contact-block-left').append(response.outputField);
                                    jQuery('#fields-list-block #fields-list-left').append(response.outputFieldSettings);
                                }
    ...
    jQuery('#fields-list-block').on('click', '.field-top-options-block>a.remove-field', function(event) {
        ...
        var inputTypeStatus = 0;
        if (jQuery(this).parents('#origin-contact-field-' + fieldId + '').attr('data-fieldType') == 'captcha') {
            inputTypeStatus = 'captcha';
        } else if (jQuery(this).parents('#origin-contact-field-' + fieldId + '').attr('data-fieldType') == 'buttons') {
            inputTypeStatus = 'buttons';
        }  else if(jQuery(this).parents('#origin-contact-field-'+fieldId+'').attr('data-fieldType')=='simple_captcha_box'){
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                if (response.removedField) {
                    ...
                    jQuery('#fields-list-block').find('li[id="origin-contact-field-' + fieldRes + '"]').fadeOut(function() {
                        jQuery(this).animate({"left": "0", "top": "0"});
                        jQuery(this).remove();
                    });
                    jQuery('#origincode-contact-wrapper').find('div[rel="origin-contact-field-' + fieldRes + '"]').fadeOut(function() {
                        jQuery(this).animate({"left": "0", "top": "0"});
                        jQuery(this).remove();
                    });
                    i = 0;
    ...
    jQuery('#fields-list-block').on('click', '.field-top-options-block>a.copy-field', function(event) {
        ...
        var fieldType = jQuery(this).parents('#origin-contact-field-' + fieldId + '').attr('data-fieldType');
        event.preventDefault();
        var form = jQuery('#adminForm');
        if (fieldType != 'custom_text') {
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    var beforeAdd = jQuery('#origincode-contact-wrapper .origincode-contact-column-block').find('div[rel="origin-contact-field-' + beforeID + '"]');
                    var beforeAddSettings = jQuery('#fields-list-block .fields-list').find('li[id="origin-contact-field-' + beforeID + '"]');
    ...
    ///SAVE FORM STARTS///
    jQuery('#origc_n_btn_block').on('click', 'input#save-buttom', function(event) {
        event.preventDefault();
        var formId = jQuery("#add-fields-block").find('li.spinnerLi').attr('data-idForm');
        var form = jQuery('#adminForm');
    ...
    jQuery('#save-button-block').on('change', 'select#select_form_theme', function(event) {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response) {
                    /*Find the icon name*/
                    var _origc_icons = response.slice(response.indexOf("originicons"),response.indexOf("{"));
                    _origc_icons = _origc_icons.split("_");
                    var _origc_res_icon= response.slice(response.indexOf("originicons", response.indexOf("#", 1)),response.indexOf("{",response.indexOf("#",1))).split("_");
                    /*Find the icon name*/
                    var style = document.getElementById('formStyles'),
                        script = document.getElementsByTagName('script')[0],
                        styles = response;
                    script.parentNode.insertBefore(style, script);
                    var regexp = /#origincode-contact-wrapper\s?>\s?div\s?>\s?h3\s?{.+?display\s?:\s?([a-zA-Z-]+)/i;
                    defaultTitleVisibility = regexp.exec(styles)[1];
                    jQuery('#select_form_show_title').trigger('change');
                    try {
                        style.innerHTML = styles;
                        /*Change icon dynamically*/
                        if(jQuery("#origincode-contact-wrapper").find("button[id^='origincode_preview_button__submit_']").length){

                            var _origc_this_btn = jQuery("button[id^='origincode_preview_button__submit_']");

                            if(_origc_this_btn.find("i").length){
                                var _origc_this_btn_i = _origc_this_btn.find("i");
                                _origc_this_btn_i = (_origc_icons[1]=="on")?_origc_this_btn_i.removeClass().addClass(_origc_icons[0]):_origc_this_btn.find("i").removeClass();

                            }
                            else {
                                if(_origc_icons[1]=="on") {
                                    _origc_this_btn.append("<i class='"+_origc_icons[0]+"'></i>");
                                }
                            }

                        }
                        if(jQuery("#origincode-contact-wrapper").find("button[id^='origincode_preview_button_reset_']").length){

                            var _origc_res_this_btn = jQuery("button[id^='origincode_preview_button_reset_']");

                            if(_origc_res_this_btn.find("i").length){
                                var _origc_res_this_btn_i = _origc_res_this_btn.find("i");
                                _origc_res_this_btn_i = (_origc_res_icon[1]=="on")?_origc_res_this_btn_i.removeClass().addClass(_origc_res_icon[0]):_origc_res_this_btn.find("i").removeClass();

                            }
                            else {
                                if(_origc_res_icon[1]=="on") {
                                    _origc_res_this_btn.append("<i class='"+_origc_res_icon[0]+"'></i>");
                                }
                            }
