function origincodeContactExportForm(JSONData,form_id) {
    ...
    var fileName = "origincode_contact_"+form_id+'_'+today;
    var uri = 'data:text/csv;charset=utf-8,' + escape(JSON.stringify(arrData));
    var link = document.createElement("a");
    link.href = uri;
    link.style = "visibility:hidden";
    link.download = fileName + ".origcf";
...
jQuery(document).ready(function () {
    jQuery(document).on('click','#export-button',function () {
        ...
        var general_data = {
            action: "origincode_contact_export_form",
            nonce: origincode_forms_exportForm.nonce,
            form: form_id
        };
        jQuery(this).find('.fa-check').removeClass('fa-check').addClass('fa-spinner');
        jQuery.post(ajaxurl, general_data, function (response) {
            if (response.success) {
                origincodeContactExportForm(response.data,form_id);
            } else {
                alert('not done');
            }
        }, "json");
